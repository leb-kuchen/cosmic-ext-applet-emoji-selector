default-{ = brace
tts-{ = open curly bracket
default-🏻 = light skin tone
tts-🏻 = light skin tone
default-🏼 = medium-light skin tone
tts-🏼 = medium-light skin tone
default-🏽 = medium skin tone
tts-🏽 = medium skin tone
default-🏾 = medium-dark skin tone
tts-🏾 = medium-dark skin tone
default-🏿 = dark skin tone
tts-🏿 = dark skin tone
default-‾ = overline
tts-‾ = overline
default-_ = dash
tts-_ = low line
default-- = dash
tts-- = hyphen-minus
default-‐ = dash
tts-‐ = hyphen
default-– = dash
tts-– = en dash
default-— = dash
tts-— = em dash
default-― = bar
tts-― = horizontal bar
default-・ = dots
tts-・ = katakana middle dot
default-, = comma
tts-, = comma
default-، = arabic
tts-، = arabic comma
default-、 = comma
tts-、 = ideographic comma
default-; = semi-colon
tts-; = semicolon
default-؛ = arabic
tts-؛ = arabic semicolon
default-: = colon
tts-: = colon
default-! = bang
tts-! = exclamation mark
default-¡ = bang
tts-¡ = inverted exclamation mark
default-? = question
tts-? = question mark
default-¿ = inverted
tts-¿ = inverted question mark
default-؟ = arabic
tts-؟ = arabic question mark
default-‽ = interabang
tts-‽ = interrobang
default-. = dot
tts-. = period
default-… = dots
tts-… = ellipsis
default-。 = full stop
tts-。 = ideographic period
default-· = dot
tts-· = middle dot
default-' = apostrophe
tts-' = typewriter apostrophe
default-‘ = apostrophe
tts-‘ = left apostrophe
default-’ = apostrophe
tts-’ = right apostrophe
default-‚ = apostrophe
tts-‚ = low right apostrophe
default-“ = double quote
tts-“ = left quotation mark
default-” = double quote
tts-” = right quotation mark
default-„ = double quote
tts-„ = low right quotation mark
default-« = angle
tts-« = left guillemet
default-» = angle
tts-» = right guillemet
default-) = bracket
tts-) = close parenthesis
default-[ = bracket
tts-[ = open square bracket
default-] = bracket
tts-] = close square bracket
default-} = brace
tts-} = close curly bracket
default-〈 = angle bracket
tts-〈 = open angle bracket
default-〉 = angle bracket
tts-〉 = close angle bracket
default-《 = bracket
tts-《 = open double angle bracket
default-》 = bracket
tts-》 = close double angle bracket
default-「 = bracket
tts-「 = open corner bracket
default-」 = bracket
tts-」 = close corner bracket
default-『 = bracket
tts-『 = open hollow corner bracket
default-』 = bracket
tts-』 = close hollow corner bracket
default-【 = bracket
tts-【 = open black lens bracket
default-】 = bracket
tts-】 = close black lens bracket
default-〔 = bracket
tts-〔 = open tortoise shell bracket
default-〕 = bracket
tts-〕 = close tortoise shell bracket
default-〖 = bracket
tts-〖 = open hollow lens bracket
default-〗 = bracket
tts-〗 = close hollow lens bracket
default-§ = paragraph
tts-§ = section
default-¶ = alinea
tts-¶ = paragraph mark
default-@ = ampersat
tts-@ = at-sign
default-* = asterisk
tts-* = asterisk
default-/ = oblique
tts-/ = slash
default-\ = backslash
tts-\ = backslash
default-& = ampersand
tts-& = ampersand
default-# = hash
tts-# = hash sign
default-% = per cent
tts-% = percent
default-‰ = per mil
tts-‰ = per mille
default-† = dagger
tts-† = dagger sign
default-‡ = dagger
tts-‡ = double dagger sign
default-• = bullet
tts-• = bullet
default-‧ = dash
tts-‧ = hyphenation point
default-′ = prime
tts-′ = prime
default-″ = double prime
tts-″ = double prime
default-‴ = prime
tts-‴ = triple prime
default-‸ = caret
tts-‸ = caret
default-※ = reference mark
tts-※ = reference mark
default-⁂ = asterism
tts-⁂ = asterism
default-` = accent
tts-` = grave accent
default-´ = accent
tts-´ = acute accent
default-^ = accent
tts-^ = circumflex accent
default-¨ = diaeresis
tts-¨ = diaeresis
default-° = degree
tts-° = degree
default-℗ = copyright
tts-℗ = sound recording copyright
default-← = arrow
tts-← = left-pointing arrow
default-↚ = leftwards arrow stroke
tts-↚ = leftwards arrow stroke
default-→ = arrow
tts-→ = right-pointing arrow
default-↛ = rightwards arrow stroke
tts-↛ = rightwards arrow stroke
default-↑ = arrow
tts-↑ = up-pointing arrow
default-↓ = arrow
tts-↓ = down-pointing arrow
default-↜ = leftwards wave arrow
tts-↜ = leftwards wave arrow
default-↝ = rightwards wave arrow
tts-↝ = rightwards wave arrow
default-↞ = leftwards two headed arrow
tts-↞ = leftwards two headed arrow
default-↟ = upwards two headed arrow
tts-↟ = upwards two headed arrow
default-↠ = rightwards two headed arrow
tts-↠ = rightwards two headed arrow
default-↡ = downwards two headed arrow
tts-↡ = downwards two headed arrow
default-↢ = leftwards arrow tail
tts-↢ = leftwards arrow tail
default-↣ = rightwards arrow tail
tts-↣ = rightwards arrow tail
default-↤ = leftwards arrow from bar
tts-↤ = leftwards arrow from bar
default-↥ = upwards arrow from bar
tts-↥ = upwards arrow from bar
default-↦ = rightwards arrow from bar
tts-↦ = rightwards arrow from bar
default-↧ = downwards arrow from bar
tts-↧ = downwards arrow from bar
default-↨ = up down arrow with base
tts-↨ = up down arrow with base
default-↫ = leftwards arrow loop
tts-↫ = leftwards arrow loop
default-↬ = rightwards arrow loop
tts-↬ = rightwards arrow loop
default-↭ = left right wave arrow
tts-↭ = left right wave arrow
default-↯ = downwards zigzag arrow
tts-↯ = downwards zigzag arrow
default-↰ = upwards arrow tip leftwards
tts-↰ = upwards arrow tip leftwards
default-↱ = upwards arrow tip rightwards
tts-↱ = upwards arrow tip rightwards
default-↲ = downwards arrow tip leftwards
tts-↲ = downwards arrow tip leftwards
default-↳ = downwards arrow tip rightwards
tts-↳ = downwards arrow tip rightwards
default-↴ = rightwards arrow corner downwards
tts-↴ = rightwards arrow corner downwards
default-↵ = downwards arrow corner leftwards
tts-↵ = downwards arrow corner leftwards
default-↶ = anticlockwise top semicircle arrow
tts-↶ = anticlockwise top semicircle arrow
default-↷ = clockwise top semicircle arrow
tts-↷ = clockwise top semicircle arrow
default-↸ = north west arrow long bar
tts-↸ = north west arrow long bar
default-↹ = arrow
tts-↹ = leftwards over rightwards arrow bars
default-↺ = anticlockwise open circle arrow
tts-↺ = anticlockwise open circle arrow
default-↻ = clockwise open circle arrow
tts-↻ = clockwise open circle arrow
default-↼ = barb
tts-↼ = leftwards harpoon barb upwards
default-↽ = leftwards harpoon barb downwards
tts-↽ = leftwards harpoon barb downwards
default-↾ = barb
tts-↾ = upwards harpoon barb rightwards
default-↿ = barb
tts-↿ = upwards harpoon barb leftwards
default-⇀ = barb
tts-⇀ = rightwards harpoon barb upwards
default-⇁ = barb
tts-⇁ = rightwards harpoon barb downwards
default-⇂ = barb
tts-⇂ = downwards harpoon barb rightwards
default-⇃ = barb
tts-⇃ = downwards harpoon barb leftwards
default-⇄ = rightwards arrow over leftwards arrow
tts-⇄ = rightwards arrow over leftwards arrow
default-⇅ = arrow
tts-⇅ = up-pointing and down-pointing arrows
default-⇆ = arrow
tts-⇆ = left-pointing over right-pointing arrows
default-⇇ = leftwards paired arrows
tts-⇇ = leftwards paired arrows
default-⇈ = upwards paired arrows
tts-⇈ = upwards paired arrows
default-⇉ = rightwards paired arrows
tts-⇉ = rightwards paired arrows
default-⇊ = downwards paired arrows
tts-⇊ = downwards paired arrows
default-⇋ = leftwards harpoon over rightwards harpoon
tts-⇋ = leftwards harpoon over rightwards harpoon
default-⇌ = rightwards harpoon over leftwards harpoon
tts-⇌ = rightwards harpoon over leftwards harpoon
default-⇐ = leftwards double arrow
tts-⇐ = leftwards double arrow
default-⇍ = leftwards double arrow stroke
tts-⇍ = leftwards double arrow stroke
default-⇑ = upwards double arrow
tts-⇑ = upwards double arrow
default-⇒ = rightwards double arrow
tts-⇒ = rightwards double arrow
default-⇏ = rightwards double arrow stroke
tts-⇏ = rightwards double arrow stroke
default-⇓ = downwards double arrow
tts-⇓ = downwards double arrow
default-⇔ = left right double arrow
tts-⇔ = left right double arrow
default-⇎ = left right double arrow stroke
tts-⇎ = left right double arrow stroke
default-⇖ = north west double arrow
tts-⇖ = north west double arrow
default-⇗ = north east double arrow
tts-⇗ = north east double arrow
default-⇘ = south east double arrow
tts-⇘ = south east double arrow
default-⇙ = south west double arrow
tts-⇙ = south west double arrow
default-⇚ = leftwards triple arrow
tts-⇚ = leftwards triple arrow
default-⇛ = rightwards triple arrow
tts-⇛ = rightwards triple arrow
default-⇜ = leftwards squiggle arrow
tts-⇜ = leftwards squiggle arrow
default-⇝ = rightwards squiggle arrow
tts-⇝ = rightwards squiggle arrow
default-⇞ = upwards arrow double stroke
tts-⇞ = upwards arrow double stroke
default-⇟ = downwards arrow double stroke
tts-⇟ = downwards arrow double stroke
default-⇠ = leftwards dashed arrow
tts-⇠ = leftwards dashed arrow
default-⇡ = upwards dashed arrow
tts-⇡ = upwards dashed arrow
default-⇢ = rightwards dashed arrow
tts-⇢ = rightwards dashed arrow
default-⇣ = downwards dashed arrow
tts-⇣ = downwards dashed arrow
default-⇤ = leftwards arrow bar
tts-⇤ = leftwards arrow bar
default-⇥ = rightwards arrow bar
tts-⇥ = rightwards arrow bar
default-⇦ = leftwards hollow arrow
tts-⇦ = leftwards hollow arrow
default-⇧ = upwards hollow arrow
tts-⇧ = upwards hollow arrow
default-⇨ = rightwards hollow arrow
tts-⇨ = rightwards hollow arrow
default-⇩ = downwards hollow arrow
tts-⇩ = downwards hollow arrow
default-⇪ = upwards hollow arrow from bar
tts-⇪ = upwards hollow arrow from bar
default-⇵ = downwards arrow leftwards upwards arrow
tts-⇵ = downwards arrow leftwards upwards arrow
default-∀ = all
tts-∀ = for all
default-∂ = differential
tts-∂ = partial differential
default-∃ = there exists
tts-∃ = there exists
default-∅ = empty set
tts-∅ = empty set
default-∆ = increment
tts-∆ = increment
default-∇ = nabla
tts-∇ = nabla
default-∈ = contains
tts-∈ = element of
default-∉ = element
tts-∉ = not an element
default-∋ = contains as member
tts-∋ = contains as member
default-∎ = end proof
tts-∎ = end proof
default-∏ = logic
tts-∏ = n-ary product
default-∑ = mathematics
tts-∑ = n-ary summation
default-+ = add
tts-+ = plus sign
default-± = plus-minus
tts-± = plus-minus
default-÷ = divide
tts-÷ = division sign
default-× = multiplication
tts-× = multiplication sign
default-< = less than
tts-< = less-than
default-≮ = inequality
tts-≮ = not less-than
default-= = equal
tts-= = equal
default-≠ = inequality
tts-≠ = not equal
default-> = close tag
tts-> = greater-than
default-≯ = inequality
tts-≯ = not greater-than
default-¬ = negation
tts-¬ = negation
default-| = bar
tts-| = vertical line
default-~ = tilde
tts-~ = tilde
default-− = minus
tts-− = minus sign
default-⁻ = minus
tts-⁻ = superscript minus
default-∓ = minus-or-plus
tts-∓ = minus-or-plus
default-∕ = division slash
tts-∕ = division slash
default-⁄ = fraction slash
tts-⁄ = fraction slash
default-∗ = asterisk operator
tts-∗ = asterisk operator
default-∘ = composition
tts-∘ = ring operator
default-∙ = bullet operator
tts-∙ = bullet operator
default-√ = radical
tts-√ = square root
default-∝ = proportional
tts-∝ = proportional
default-∞ = infinity
tts-∞ = infinity sign
default-∟ = mathematics
tts-∟ = right angle
default-∠ = acute
tts-∠ = angle
default-∣ = divides
tts-∣ = divides
default-∥ = parallel
tts-∥ = parallel
default-∧ = ac
tts-∧ = logical and
default-∩ = intersection
tts-∩ = intersection
default-∪ = collection
tts-∪ = union
default-∫ = calculus
tts-∫ = integral
default-∬ = calculus
tts-∬ = double integral
default-∮ = contour integral
tts-∮ = contour integral
default-∴ = logic
tts-∴ = therefore
default-∵ = because
tts-∵ = because
default-∶ = ratio
tts-∶ = ratio
default-∷ = proportion
tts-∷ = proportion
default-∼ = operator
tts-∼ = tilde operator
default-∽ = reversed tilde
tts-∽ = reversed tilde
default-∾ = inverted lazy s
tts-∾ = inverted lazy s
default-≃ = asymptote
tts-≃ = asymptotically equal
default-≅ = approximately equal
tts-≅ = approximately equal
default-≈ = almost equal
tts-≈ = almost equal
default-≌ = all equal
tts-≌ = all equal
default-≒ = approximately equal the image
tts-≒ = approximately equal the image
default-≖ = equality
tts-≖ = ring in equal
default-≡ = exact
tts-≡ = identical to
default-≣ = equality
tts-≣ = strictly equivalent
default-≤ = equal
tts-≤ = less-than or equal
default-≥ = equal
tts-≥ = greater-than or equal
default-≦ = inequality
tts-≦ = less-than over equal
default-≧ = greater-than over equal
tts-≧ = greater-than over equal
default-≪ = inequality
tts-≪ = much less-than
default-≫ = inequality
tts-≫ = much greater-than
default-≬ = between
tts-≬ = between
default-≳ = greater-than equivalent
tts-≳ = greater-than equivalent
default-≺ = mathematics
tts-≺ = precedes
default-≻ = mathematics
tts-≻ = succeeds
default-⊁ = does not succeed
tts-⊁ = does not succeed
default-⊂ = set
tts-⊂ = subset of
default-⊃ = mathematics
tts-⊃ = superset
default-⊆ = subset equal
tts-⊆ = subset equal
default-⊇ = equality
tts-⊇ = superset equal
default-⊕ = circled plus
tts-⊕ = circled plus
default-⊖ = circled minus
tts-⊖ = circled minus
default-⊗ = circled times
tts-⊗ = circled times
default-⊘ = circled division slash
tts-⊘ = circled division slash
default-⊙ = circled dot operator
tts-⊙ = circled dot operator
default-⊚ = circled ring operator
tts-⊚ = circled ring operator
default-⊛ = asterisk
tts-⊛ = circled asterisk operator
default-⊞ = addition-like
tts-⊞ = squared plus
default-⊟ = mathematics
tts-⊟ = squared minus
default-⊥ = eet
tts-⊥ = up tack
default-⊮ = does not force
tts-⊮ = does not force
default-⊰ = mathematics
tts-⊰ = precedes under relation
default-⊱ = mathematics
tts-⊱ = succeeds under relation
default-⋭ = does not contain as normal subgroup equal
tts-⋭ = does not contain as normal subgroup equal
default-⊶ = original
tts-⊶ = original
default-⊹ = hermitian conjugate matrix
tts-⊹ = hermitian conjugate matrix
default-⊿ = mathematics
tts-⊿ = right triangle
default-⋁ = disjunction
tts-⋁ = n-ary logical or
default-⋂ = intersection
tts-⋂ = n-ary intersection
default-⋃ = mathematics
tts-⋃ = n-ary union
default-⋅ = dot operator
tts-⋅ = dot operator
default-⋆ = operator
tts-⋆ = star operator
default-⋈ = binary operator
tts-⋈ = natural join
default-⋒ = double intersection
tts-⋒ = double intersection
default-⋘ = inequality
tts-⋘ = very much less-than
default-⋙ = inequality
tts-⋙ = very much greater-than
default-⋮ = ellipsis
tts-⋮ = vertical ellipsis
default-⋯ = ellipsis
tts-⋯ = midline horizontal ellipsis
default-⋰ = ellipsis
tts-⋰ = up right diagonal ellipsis
default-⋱ = down right diagonal ellipsis
tts-⋱ = down right diagonal ellipsis
default-■ = filled square
tts-■ = filled square
default-□ = hollow square
tts-□ = hollow square
default-▢ = hollow square with rounded corners
tts-▢ = hollow square with rounded corners
default-▣ = hollow square containing filled square
tts-▣ = hollow square containing filled square
default-▤ = square with horizontal fill
tts-▤ = square with horizontal fill
default-▥ = square with vertical fill
tts-▥ = square with vertical fill
default-▦ = square orthogonal crosshatch fill
tts-▦ = square orthogonal crosshatch fill
default-▧ = square upper left lower right fill
tts-▧ = square upper left lower right fill
default-▨ = square upper right lower left fill
tts-▨ = square upper right lower left fill
default-▩ = square diagonal crosshatch fill
tts-▩ = square diagonal crosshatch fill
default-▬ = filled rectangle
tts-▬ = filled rectangle
default-▭ = hollow rectangle
tts-▭ = hollow rectangle
default-▮ = filled vertical rectangle
tts-▮ = filled vertical rectangle
default-▰ = filled parallelogram
tts-▰ = filled parallelogram
default-▲ = arrow
tts-▲ = filled up-pointing triangle
default-△ = hollow up-pointing triangle
tts-△ = hollow up-pointing triangle
default-▴ = filled up-pointing small triangle
tts-▴ = filled up-pointing small triangle
default-▵ = hollow up-pointing small triangle
tts-▵ = hollow up-pointing small triangle
default-▷ = hollow right-pointing triangle
tts-▷ = hollow right-pointing triangle
default-▸ = filled right-pointing small triangle
tts-▸ = filled right-pointing small triangle
default-▹ = hollow right-pointing small triangle
tts-▹ = hollow right-pointing small triangle
default-► = filled right-pointing pointer
tts-► = filled right-pointing pointer
default-▻ = hollow right-pointing pointer
tts-▻ = hollow right-pointing pointer
default-▼ = arrow
tts-▼ = filled down-pointing triangle
default-▽ = hollow down-pointing triangle
tts-▽ = hollow down-pointing triangle
default-▾ = filled down-pointing small triangle
tts-▾ = filled down-pointing small triangle
default-▿ = hollow down-pointing small triangle
tts-▿ = hollow down-pointing small triangle
default-◁ = hollow left-pointing triangle
tts-◁ = hollow left-pointing triangle
default-◂ = filled left-pointing small triangle
tts-◂ = filled left-pointing small triangle
default-◃ = hollow left-pointing small triangle
tts-◃ = hollow left-pointing small triangle
default-◄ = filled left-pointing pointer
tts-◄ = filled left-pointing pointer
default-◅ = hollow left-pointing pointer
tts-◅ = hollow left-pointing pointer
default-◆ = filled diamond
tts-◆ = filled diamond
default-◇ = hollow diamond
tts-◇ = hollow diamond
default-◈ = hollow diamond containing filled diamond
tts-◈ = hollow diamond containing filled diamond
default-◉ = circled dot
tts-◉ = hollow circle containing filled circle
default-◊ = diamond
tts-◊ = lozenge
default-○ = circle
tts-○ = hollow circle
default-◌ = dotted circle
tts-◌ = dotted circle
default-◍ = circle with vertical fill
tts-◍ = circle with vertical fill
default-◎ = concentric circles
tts-◎ = concentric circles
default-● = circle
tts-● = filled circle
default-◐ = circle left half filled
tts-◐ = circle left half filled
default-◑ = circle right half filled
tts-◑ = circle right half filled
default-◒ = circle lower half filled
tts-◒ = circle lower half filled
default-◓ = circle upper half filled
tts-◓ = circle upper half filled
default-◔ = circle upper right quadrant filled
tts-◔ = circle upper right quadrant filled
default-◕ = circle all but upper left quadrant filled
tts-◕ = circle all but upper left quadrant filled
default-◖ = left half filled circle
tts-◖ = left half filled circle
default-◗ = right half filled circle
tts-◗ = right half filled circle
default-◘ = inverse bullet
tts-◘ = inverse bullet
default-◙ = filled square containing hollow circle
tts-◙ = filled square containing hollow circle
default-◜ = upper left quadrant circular arc
tts-◜ = upper left quadrant circular arc
default-◝ = upper right quadrant circular arc
tts-◝ = upper right quadrant circular arc
default-◞ = lower right quadrant circular arc
tts-◞ = lower right quadrant circular arc
default-◟ = lower left quadrant circular arc
tts-◟ = lower left quadrant circular arc
default-◠ = upper half circle
tts-◠ = upper half circle
default-◡ = lower half circle
tts-◡ = lower half circle
default-◢ = filled lower right triangle
tts-◢ = filled lower right triangle
default-◣ = filled lower left triangle
tts-◣ = filled lower left triangle
default-◤ = filled upper left triangle
tts-◤ = filled upper left triangle
default-◥ = filled upper right triangle
tts-◥ = filled upper right triangle
default-◦ = hollow bullet
tts-◦ = hollow bullet
default-◯ = circle
tts-◯ = large hollow circle
default-◳ = hollow square upper right quadrant
tts-◳ = hollow square upper right quadrant
default-◷ = hollow circle with upper right quadrant
tts-◷ = hollow circle with upper right quadrant
default-◿ = lower right triangle
tts-◿ = lower right triangle
default-♪ = eighth
tts-♪ = eighth note
default-⨧ = plus subscript two
tts-⨧ = plus subscript two
default-⨯ = vector cross product
tts-⨯ = vector cross product
default-⨼ = interior product
tts-⨼ = interior product
default-⩣ = logical or double underbar
tts-⩣ = logical or double underbar
default-⩽ = less-than slanted equal
tts-⩽ = less-than slanted equal
default-⪍ = less-than above similar equal
tts-⪍ = less-than above similar equal
default-⪚ = double-line equal greater-than
tts-⪚ = double-line equal greater-than
default-⪺ = succeeds above not almost equal
tts-⪺ = succeeds above not almost equal
default-♭ = bemolle
tts-♭ = flat
default-♯ = dièse
tts-♯ = sharp
default-😀 = face
tts-😀 = grinning face
default-😃 = face
tts-😃 = grinning face with big eyes
default-😄 = eye
tts-😄 = grinning face with smiling eyes
default-😁 = beaming face with smiling eyes
tts-😁 = beaming face with smiling eyes
default-😆 = face
tts-😆 = grinning squinting face
default-😅 = cold
tts-😅 = grinning face with sweat
default-🤣 = face
tts-🤣 = rolling on the floor laughing
default-😂 = face
tts-😂 = face with tears of joy
default-🙂 = face
tts-🙂 = slightly smiling face
default-🙃 = face
tts-🙃 = upside-down face
default-🫠 = disappear
tts-🫠 = melting face
default-😉 = face
tts-😉 = winking face
default-😊 = blush
tts-😊 = smiling face with smiling eyes
default-😇 = angel
tts-😇 = smiling face with halo
default-🥰 = adore
tts-🥰 = smiling face with hearts
default-😍 = eye
tts-😍 = smiling face with heart-eyes
default-🤩 = eyes
tts-🤩 = star-struck
default-😘 = face
tts-😘 = face blowing a kiss
default-😗 = face
tts-😗 = kissing face
default-☺ = face
tts-☺ = smiling face
default-😚 = closed
tts-😚 = kissing face with closed eyes
default-😙 = eye
tts-😙 = kissing face with smiling eyes
default-🥲 = grateful
tts-🥲 = smiling face with tear
default-😋 = delicious
tts-😋 = face savoring food
default-😛 = face
tts-😛 = face with tongue
default-😜 = eye
tts-😜 = winking face with tongue
default-🤪 = eye
tts-🤪 = zany face
default-😝 = eye
tts-😝 = squinting face with tongue
default-🤑 = face
tts-🤑 = money-mouth face
default-🤗 = face
tts-🤗 = smiling face with open hands
default-🤭 = face with hand over mouth
tts-🤭 = face with hand over mouth
default-🫢 = amazement
tts-🫢 = face with open eyes and hand over mouth
default-🫣 = captivated
tts-🫣 = face with peeking eye
default-🤫 = quiet
tts-🤫 = shushing face
default-🤔 = face
tts-🤔 = thinking face
default-🫡 = OK
tts-🫡 = saluting face
default-🤐 = face
tts-🤐 = zipper-mouth face
default-🤨 = distrust
tts-🤨 = face with raised eyebrow
default-😐 = deadpan
tts-😐 = neutral face
default-😑 = expressionless
tts-😑 = expressionless face
default-😶 = face
tts-😶 = face without mouth
default-🫥 = depressed
tts-🫥 = dotted line face
default-😶‍🌫 = absentminded
tts-😶‍🌫 = face in clouds
default-😏 = face
tts-😏 = smirking face
default-😒 = face
tts-😒 = unamused face
default-🙄 = eyeroll
tts-🙄 = face with rolling eyes
default-😬 = face
tts-😬 = grimacing face
default-😮‍💨 = exhale
tts-😮‍💨 = face exhaling
default-🤥 = face
tts-🤥 = lying face
default-🫨 = earthquake
tts-🫨 = shaking face
default-🙂‍↔ = head shaking horizontally
tts-🙂‍↔ = head shaking horizontally
default-🙂‍↕ = head shaking vertically
tts-🙂‍↕ = head shaking vertically
default-😌 = face
tts-😌 = relieved face
default-😔 = dejected
tts-😔 = pensive face
default-😪 = face
tts-😪 = sleepy face
default-🤤 = drooling
tts-🤤 = drooling face
default-😴 = face
tts-😴 = sleeping face
default-😷 = cold
tts-😷 = face with medical mask
default-🤒 = face
tts-🤒 = face with thermometer
default-🤕 = bandage
tts-🤕 = face with head-bandage
default-🤢 = face
tts-🤢 = nauseated face
default-🤮 = face vomiting
tts-🤮 = face vomiting
default-🤧 = face
tts-🤧 = sneezing face
default-🥵 = feverish
tts-🥵 = hot face
default-🥶 = blue-faced
tts-🥶 = cold face
default-🥴 = dizzy
tts-🥴 = woozy face
default-😵 = crossed-out eyes
tts-😵 = face with crossed-out eyes
default-😵‍💫 = dizzy
tts-😵‍💫 = face with spiral eyes
default-🤯 = exploding head
tts-🤯 = exploding head
default-🤠 = cowboy
tts-🤠 = cowboy hat face
default-🥳 = celebration
tts-🥳 = partying face
default-🥸 = disguise
tts-🥸 = disguised face
default-😎 = bright
tts-😎 = smiling face with sunglasses
default-🤓 = face
tts-🤓 = nerd face
default-🧐 = face
tts-🧐 = face with monocle
default-😕 = confused
tts-😕 = confused face
default-🫤 = disappointed
tts-🫤 = face with diagonal mouth
default-😟 = face
tts-😟 = worried face
default-🙁 = face
tts-🙁 = slightly frowning face
default-☹ = face
tts-☹ = frowning face
default-😮 = face
tts-😮 = face with open mouth
default-😯 = face
tts-😯 = hushed face
default-😲 = astonished
tts-😲 = astonished face
default-😳 = dazed
tts-😳 = flushed face
default-🥺 = begging
tts-🥺 = pleading face
default-🥹 = angry
tts-🥹 = face holding back tears
default-😦 = face
tts-😦 = frowning face with open mouth
default-😧 = anguished
tts-😧 = anguished face
default-😨 = face
tts-😨 = fearful face
default-😰 = anxious face with sweat
tts-😰 = anxious face with sweat
default-😥 = disappointed
tts-😥 = sad but relieved face
default-😢 = cry
tts-😢 = crying face
default-😭 = cry
tts-😭 = loudly crying face
default-😱 = face
tts-😱 = face screaming in fear
default-😖 = confounded
tts-😖 = confounded face
default-😣 = face
tts-😣 = persevering face
default-😞 = disappointed
tts-😞 = disappointed face
default-😓 = cold
tts-😓 = downcast face with sweat
default-😩 = face
tts-😩 = weary face
default-😫 = face
tts-😫 = tired face
default-🥱 = bored
tts-🥱 = yawning face
default-😤 = face
tts-😤 = face with steam from nose
default-😡 = angry
tts-😡 = enraged face
default-😠 = anger
tts-😠 = angry face
default-🤬 = face with symbols on mouth
tts-🤬 = face with symbols on mouth
default-😈 = face
tts-😈 = smiling face with horns
default-👿 = angry face with horns
tts-👿 = angry face with horns
default-💀 = death
tts-💀 = skull
default-☠ = crossbones
tts-☠ = skull and crossbones
default-💩 = dung
tts-💩 = pile of poo
default-🤡 = clown
tts-🤡 = clown face
default-👹 = creature
tts-👹 = ogre
default-👺 = creature
tts-👺 = goblin
default-👻 = creature
tts-👻 = ghost
default-👽 = alien
tts-👽 = alien
default-👾 = alien
tts-👾 = alien monster
default-🤖 = face
tts-🤖 = robot
default-😺 = cat
tts-😺 = grinning cat
default-😸 = cat
tts-😸 = grinning cat with smiling eyes
default-😹 = cat
tts-😹 = cat with tears of joy
default-😻 = cat
tts-😻 = smiling cat with heart-eyes
default-😼 = cat
tts-😼 = cat with wry smile
default-😽 = cat
tts-😽 = kissing cat
default-🙀 = cat
tts-🙀 = weary cat
default-😿 = cat
tts-😿 = crying cat
default-😾 = cat
tts-😾 = pouting cat
default-🙈 = evil
tts-🙈 = see-no-evil monkey
default-🙉 = evil
tts-🙉 = hear-no-evil monkey
default-🙊 = evil
tts-🙊 = speak-no-evil monkey
default-💌 = heart
tts-💌 = love letter
default-💘 = arrow
tts-💘 = heart with arrow
default-💝 = heart with ribbon
tts-💝 = heart with ribbon
default-💖 = excited
tts-💖 = sparkling heart
default-💗 = excited
tts-💗 = growing heart
default-💓 = beating
tts-💓 = beating heart
default-💞 = revolving
tts-💞 = revolving hearts
default-💕 = love
tts-💕 = two hearts
default-💟 = heart
tts-💟 = heart decoration
default-❣ = exclamation
tts-❣ = heart exclamation
default-💔 = break
tts-💔 = broken heart
default-❤‍🔥 = burn
tts-❤‍🔥 = heart on fire
default-❤‍🩹 = healthier
tts-❤‍🩹 = mending heart
default-❤ = heart
tts-❤ = red heart
default-🩷 = cute
tts-🩷 = pink heart
default-🧡 = orange
tts-🧡 = orange heart
default-💛 = yellow
tts-💛 = yellow heart
default-💚 = green
tts-💚 = green heart
default-💙 = blue
tts-💙 = blue heart
default-🩵 = cyan
tts-🩵 = light blue heart
default-💜 = purple
tts-💜 = purple heart
default-🤎 = brown
tts-🤎 = brown heart
default-🖤 = black
tts-🖤 = black heart
default-🩶 = gray
tts-🩶 = grey heart
default-🤍 = heart
tts-🤍 = white heart
default-💋 = kiss
tts-💋 = kiss mark
default-💯 = 100
tts-💯 = hundred points
default-💢 = anger symbol
tts-💢 = anger symbol
default-💥 = boom
tts-💥 = collision
default-💫 = comic
tts-💫 = dizzy
default-💦 = comic
tts-💦 = sweat droplets
default-💨 = comic
tts-💨 = dashing away
default-🕳 = hole
tts-🕳 = hole
default-💬 = balloon
tts-💬 = speech balloon
default-👁‍🗨 = balloon
tts-👁‍🗨 = eye in speech bubble
default-🗨 = balloon
tts-🗨 = left speech bubble
default-🗯 = angry
tts-🗯 = right anger bubble
default-💭 = balloon
tts-💭 = thought balloon
default-💤 = comic
tts-💤 = ZZZ
default-👋 = hand
tts-👋 = waving hand
default-🤚 = backhand
tts-🤚 = raised back of hand
default-🖐 = finger
tts-🖐 = hand with fingers splayed
default-✋ = hand
tts-✋ = raised hand
default-🖖 = finger
tts-🖖 = vulcan salute
default-🫱 = hand
tts-🫱 = rightwards hand
default-🫲 = hand
tts-🫲 = leftwards hand
default-🫳 = dismiss
tts-🫳 = palm down hand
default-🫴 = beckon
tts-🫴 = palm up hand
default-🫷 = high five
tts-🫷 = leftwards pushing hand
default-🫸 = high five
tts-🫸 = rightwards pushing hand
default-👌 = hand
tts-👌 = OK hand
default-🤌 = fingers
tts-🤌 = pinched fingers
default-🤏 = pinching hand
tts-🤏 = pinching hand
default-✌ = hand
tts-✌ = victory hand
default-🤞 = cross
tts-🤞 = crossed fingers
default-🫰 = expensive
tts-🫰 = hand with index finger and thumb crossed
default-🤟 = hand
tts-🤟 = love-you gesture
default-🤘 = finger
tts-🤘 = sign of the horns
default-🤙 = call
tts-🤙 = call me hand
default-👈 = backhand
tts-👈 = backhand index pointing left
default-👉 = backhand
tts-👉 = backhand index pointing right
default-👆 = backhand
tts-👆 = backhand index pointing up
default-🖕 = finger
tts-🖕 = middle finger
default-👇 = backhand
tts-👇 = backhand index pointing down
default-☝ = finger
tts-☝ = index pointing up
default-🫵 = index pointing at the viewer
tts-🫵 = index pointing at the viewer
default-👍 = +1
tts-👍 = thumbs up
default-👎 = -1
tts-👎 = thumbs down
default-✊ = clenched
tts-✊ = raised fist
default-👊 = clenched
tts-👊 = oncoming fist
default-🤛 = fist
tts-🤛 = left-facing fist
default-🤜 = fist
tts-🤜 = right-facing fist
default-👏 = clap
tts-👏 = clapping hands
default-🙌 = celebration
tts-🙌 = raising hands
default-🫶 = heart hands
tts-🫶 = heart hands
default-👐 = hand
tts-👐 = open hands
default-🤲 = palms up together
tts-🤲 = palms up together
default-🤝 = agreement
tts-🤝 = handshake
default-🙏 = ask
tts-🙏 = folded hands
default-✍ = hand
tts-✍ = writing hand
default-💅 = care
tts-💅 = nail polish
default-🤳 = camera
tts-🤳 = selfie
default-💪 = biceps
tts-💪 = flexed biceps
default-🦾 = accessibility
tts-🦾 = mechanical arm
default-🦿 = accessibility
tts-🦿 = mechanical leg
default-🦵 = kick
tts-🦵 = leg
default-🦶 = foot
tts-🦶 = foot
default-👂 = body
tts-👂 = ear
default-🦻 = accessibility
tts-🦻 = ear with hearing aid
default-👃 = body
tts-👃 = nose
default-🧠 = brain
tts-🧠 = brain
default-🫀 = anatomical
tts-🫀 = anatomical heart
default-🫁 = breath
tts-🫁 = lungs
default-🦷 = dentist
tts-🦷 = tooth
default-🦴 = bone
tts-🦴 = bone
default-👀 = eye
tts-👀 = eyes
default-👁 = body
tts-👁 = eye
default-👅 = body
tts-👅 = tongue
default-👄 = lips
tts-👄 = mouth
default-🫦 = anxious
tts-🫦 = biting lip
default-👶 = baby
tts-👶 = baby
default-🧒 = child
tts-🧒 = child
default-👦 = boy
tts-👦 = boy
default-👧 = girl
tts-👧 = girl
default-🧑 = adult
tts-🧑 = person
default-👱 = blond
tts-👱 = person: blond hair
default-👨 = adult
tts-👨 = man
default-🧔 = beard
tts-🧔 = person: beard
default-🧔‍♂ = beard
tts-🧔‍♂ = man: beard
default-👱‍♂ = blond
tts-👱‍♂ = man: blond hair
default-👩 = adult
tts-👩 = woman
default-🧔‍♀ = beard
tts-🧔‍♀ = woman: beard
default-👱‍♀ = blond-haired woman
tts-👱‍♀ = woman: blond hair
default-🧓 = adult
tts-🧓 = older person
default-👴 = adult
tts-👴 = old man
default-👵 = adult
tts-👵 = old woman
default-🙍 = frown
tts-🙍 = person frowning
default-🙍‍♂ = frowning
tts-🙍‍♂ = man frowning
default-🙍‍♀ = frowning
tts-🙍‍♀ = woman frowning
default-🙎 = gesture
tts-🙎 = person pouting
default-🙎‍♂ = gesture
tts-🙎‍♂ = man pouting
default-🙎‍♀ = gesture
tts-🙎‍♀ = woman pouting
default-🙅 = forbidden
tts-🙅 = person gesturing NO
default-🙅‍♂ = forbidden
tts-🙅‍♂ = man gesturing NO
default-🙅‍♀ = forbidden
tts-🙅‍♀ = woman gesturing NO
default-🙆 = gesture
tts-🙆 = person gesturing OK
default-🙆‍♂ = gesture
tts-🙆‍♂ = man gesturing OK
default-🙆‍♀ = gesture
tts-🙆‍♀ = woman gesturing OK
default-💁 = hand
tts-💁 = person tipping hand
default-💁‍♂ = man
tts-💁‍♂ = man tipping hand
default-💁‍♀ = sassy
tts-💁‍♀ = woman tipping hand
default-🙋 = gesture
tts-🙋 = person raising hand
default-🙋‍♂ = gesture
tts-🙋‍♂ = man raising hand
default-🙋‍♀ = gesture
tts-🙋‍♀ = woman raising hand
default-🧏 = accessibility
tts-🧏 = deaf person
default-🧏‍♂ = deaf
tts-🧏‍♂ = deaf man
default-🧏‍♀ = deaf
tts-🧏‍♀ = deaf woman
default-🙇 = apology
tts-🙇 = person bowing
default-🙇‍♂ = apology
tts-🙇‍♂ = man bowing
default-🙇‍♀ = apology
tts-🙇‍♀ = woman bowing
default-🤦 = disbelief
tts-🤦 = person facepalming
default-🤦‍♂ = disbelief
tts-🤦‍♂ = man facepalming
default-🤦‍♀ = disbelief
tts-🤦‍♀ = woman facepalming
default-🤷 = doubt
tts-🤷 = person shrugging
default-🤷‍♂ = doubt
tts-🤷‍♂ = man shrugging
default-🤷‍♀ = doubt
tts-🤷‍♀ = woman shrugging
default-🧑‍⚕ = doctor
tts-🧑‍⚕ = health worker
default-👨‍⚕ = doctor
tts-👨‍⚕ = man health worker
default-👩‍⚕ = doctor
tts-👩‍⚕ = woman health worker
default-🧑‍🎓 = graduate
tts-🧑‍🎓 = student
default-👨‍🎓 = graduate
tts-👨‍🎓 = man student
default-👩‍🎓 = graduate
tts-👩‍🎓 = woman student
default-🧑‍🏫 = instructor
tts-🧑‍🏫 = teacher
default-👨‍🏫 = instructor
tts-👨‍🏫 = man teacher
default-👩‍🏫 = instructor
tts-👩‍🏫 = woman teacher
default-🧑‍⚖ = judge
tts-🧑‍⚖ = judge
default-👨‍⚖ = judge
tts-👨‍⚖ = man judge
default-👩‍⚖ = judge
tts-👩‍⚖ = woman judge
default-🧑‍🌾 = farmer
tts-🧑‍🌾 = farmer
default-👨‍🌾 = farmer
tts-👨‍🌾 = man farmer
default-👩‍🌾 = farmer
tts-👩‍🌾 = woman farmer
default-🧑‍🍳 = chef
tts-🧑‍🍳 = cook
default-👨‍🍳 = chef
tts-👨‍🍳 = man cook
default-👩‍🍳 = chef
tts-👩‍🍳 = woman cook
default-🧑‍🔧 = electrician
tts-🧑‍🔧 = mechanic
default-👨‍🔧 = electrician
tts-👨‍🔧 = man mechanic
default-👩‍🔧 = electrician
tts-👩‍🔧 = woman mechanic
default-🧑‍🏭 = assembly
tts-🧑‍🏭 = factory worker
default-👨‍🏭 = assembly
tts-👨‍🏭 = man factory worker
default-👩‍🏭 = assembly
tts-👩‍🏭 = woman factory worker
default-🧑‍💼 = architect
tts-🧑‍💼 = office worker
default-👨‍💼 = architect
tts-👨‍💼 = man office worker
default-👩‍💼 = architect
tts-👩‍💼 = woman office worker
default-🧑‍🔬 = biologist
tts-🧑‍🔬 = scientist
default-👨‍🔬 = biologist
tts-👨‍🔬 = man scientist
default-👩‍🔬 = biologist
tts-👩‍🔬 = woman scientist
default-🧑‍💻 = coder
tts-🧑‍💻 = technologist
default-👨‍💻 = coder
tts-👨‍💻 = man technologist
default-👩‍💻 = coder
tts-👩‍💻 = woman technologist
default-🧑‍🎤 = actor
tts-🧑‍🎤 = singer
default-👨‍🎤 = actor
tts-👨‍🎤 = man singer
default-👩‍🎤 = actor
tts-👩‍🎤 = woman singer
default-🧑‍🎨 = artist
tts-🧑‍🎨 = artist
default-👨‍🎨 = artist
tts-👨‍🎨 = man artist
default-👩‍🎨 = artist
tts-👩‍🎨 = woman artist
default-🧑‍✈ = pilot
tts-🧑‍✈ = pilot
default-👨‍✈ = man
tts-👨‍✈ = man pilot
default-👩‍✈ = pilot
tts-👩‍✈ = woman pilot
default-🧑‍🚀 = astronaut
tts-🧑‍🚀 = astronaut
default-👨‍🚀 = astronaut
tts-👨‍🚀 = man astronaut
default-👩‍🚀 = astronaut
tts-👩‍🚀 = woman astronaut
default-🧑‍🚒 = fire
tts-🧑‍🚒 = firefighter
default-👨‍🚒 = firefighter
tts-👨‍🚒 = man firefighter
default-👩‍🚒 = firefighter
tts-👩‍🚒 = woman firefighter
default-👮 = cop
tts-👮 = police officer
default-👮‍♂ = cop
tts-👮‍♂ = man police officer
default-👮‍♀ = cop
tts-👮‍♀ = woman police officer
default-🕵 = detective
tts-🕵 = detective
default-🕵‍♂ = detective
tts-🕵‍♂ = man detective
default-🕵‍♀ = detective
tts-🕵‍♀ = woman detective
default-💂 = guard
tts-💂 = guard
default-💂‍♂ = guard
tts-💂‍♂ = man guard
default-💂‍♀ = guard
tts-💂‍♀ = woman guard
default-🥷 = fighter
tts-🥷 = ninja
default-👷 = construction
tts-👷 = construction worker
default-👷‍♂ = construction
tts-👷‍♂ = man construction worker
default-👷‍♀ = construction
tts-👷‍♀ = woman construction worker
default-🫅 = monarch
tts-🫅 = person with crown
default-🤴 = prince
tts-🤴 = prince
default-👸 = fairy tale
tts-👸 = princess
default-👳 = person wearing turban
tts-👳 = person wearing turban
default-👳‍♂ = man
tts-👳‍♂ = man wearing turban
default-👳‍♀ = turban
tts-👳‍♀ = woman wearing turban
default-👲 = cap
tts-👲 = person with skullcap
default-🧕 = headscarf
tts-🧕 = woman with headscarf
default-🤵 = groom
tts-🤵 = person in tuxedo
default-🤵‍♂ = man
tts-🤵‍♂ = man in tuxedo
default-🤵‍♀ = tuxedo
tts-🤵‍♀ = woman in tuxedo
default-👰 = bride
tts-👰 = person with veil
default-👰‍♂ = man
tts-👰‍♂ = man with veil
default-👰‍♀ = veil
tts-👰‍♀ = woman with veil
default-🤰 = pregnant
tts-🤰 = pregnant woman
default-🫃 = belly
tts-🫃 = pregnant man
default-🫄 = belly
tts-🫄 = pregnant person
default-🤱 = baby
tts-🤱 = breast-feeding
default-👩‍🍼 = baby
tts-👩‍🍼 = woman feeding baby
default-👨‍🍼 = baby
tts-👨‍🍼 = man feeding baby
default-🧑‍🍼 = baby
tts-🧑‍🍼 = person feeding baby
default-👼 = angel
tts-👼 = baby angel
default-🎅 = celebration
tts-🎅 = Santa Claus
default-🤶 = celebration
tts-🤶 = Mrs. Claus
default-🧑‍🎄 = christmas
tts-🧑‍🎄 = mx claus
default-🦸 = good
tts-🦸 = superhero
default-🦸‍♂ = good
tts-🦸‍♂ = man superhero
default-🦸‍♀ = good
tts-🦸‍♀ = woman superhero
default-🦹 = criminal
tts-🦹 = supervillain
default-🦹‍♂ = criminal
tts-🦹‍♂ = man supervillain
default-🦹‍♀ = criminal
tts-🦹‍♀ = woman supervillain
default-🧙 = mage
tts-🧙 = mage
default-🧙‍♂ = man mage
tts-🧙‍♂ = man mage
default-🧙‍♀ = sorceress
tts-🧙‍♀ = woman mage
default-🧚 = fairy
tts-🧚 = fairy
default-🧚‍♂ = man fairy
tts-🧚‍♂ = man fairy
default-🧚‍♀ = Titania
tts-🧚‍♀ = woman fairy
default-🧛 = Dracula
tts-🧛 = vampire
default-🧛‍♂ = Dracula
tts-🧛‍♂ = man vampire
default-🧛‍♀ = undead
tts-🧛‍♀ = woman vampire
default-🧜 = mermaid
tts-🧜 = merperson
default-🧜‍♂ = merman
tts-🧜‍♂ = merman
default-🧜‍♀ = mermaid
tts-🧜‍♀ = mermaid
default-🧝 = elf
tts-🧝 = elf
default-🧝‍♂ = magical
tts-🧝‍♂ = man elf
default-🧝‍♀ = magical
tts-🧝‍♀ = woman elf
default-🧞 = djinn
tts-🧞 = genie
default-🧞‍♂ = djinn
tts-🧞‍♂ = man genie
default-🧞‍♀ = djinn
tts-🧞‍♀ = woman genie
default-🧟 = undead
tts-🧟 = zombie
default-🧟‍♂ = man zombie
tts-🧟‍♂ = man zombie
default-🧟‍♀ = undead
tts-🧟‍♀ = woman zombie
default-🧌 = fairy tale
tts-🧌 = troll
default-💆 = face
tts-💆 = person getting massage
default-💆‍♂ = face
tts-💆‍♂ = man getting massage
default-💆‍♀ = face
tts-💆‍♀ = woman getting massage
default-💇 = barber
tts-💇 = person getting haircut
default-💇‍♂ = haircut
tts-💇‍♂ = man getting haircut
default-💇‍♀ = haircut
tts-💇‍♀ = woman getting haircut
default-🚶 = hike
tts-🚶 = person walking
default-🚶‍♂ = hike
tts-🚶‍♂ = man walking
default-🚶‍♀ = hike
tts-🚶‍♀ = woman walking
default-🧍 = person standing
tts-🧍 = person standing
default-🧍‍♂ = man
tts-🧍‍♂ = man standing
default-🧍‍♀ = standing
tts-🧍‍♀ = woman standing
default-🧎 = kneel
tts-🧎 = person kneeling
default-🧎‍♂ = kneeling
tts-🧎‍♂ = man kneeling
default-🧎‍♀ = kneeling
tts-🧎‍♀ = woman kneeling
default-🧑‍🦯 = accessibility
tts-🧑‍🦯 = person with white cane
default-👨‍🦯 = accessibility
tts-👨‍🦯 = man with white cane
default-👩‍🦯 = accessibility
tts-👩‍🦯 = woman with white cane
default-🧑‍🦼 = accessibility
tts-🧑‍🦼 = person in motorized wheelchair
default-👨‍🦼 = accessibility
tts-👨‍🦼 = man in motorized wheelchair
default-👩‍🦼 = accessibility
tts-👩‍🦼 = woman in motorized wheelchair
default-🧑‍🦽 = accessibility
tts-🧑‍🦽 = person in manual wheelchair
default-👨‍🦽 = accessibility
tts-👨‍🦽 = man in manual wheelchair
default-👩‍🦽 = accessibility
tts-👩‍🦽 = woman in manual wheelchair
default-🏃 = marathon
tts-🏃 = person running
default-🏃‍♂ = man
tts-🏃‍♂ = man running
default-🏃‍♀ = marathon
tts-🏃‍♀ = woman running
default-💃 = dance
tts-💃 = woman dancing
default-🕺 = dance
tts-🕺 = man dancing
default-🕴 = business
tts-🕴 = person in suit levitating
default-👯 = bunny ear
tts-👯 = people with bunny ears
default-👯‍♂ = bunny ear
tts-👯‍♂ = men with bunny ears
default-👯‍♀ = bunny ear
tts-👯‍♀ = women with bunny ears
default-🧖 = person in steamy room
tts-🧖 = person in steamy room
default-🧖‍♂ = man in steamy room
tts-🧖‍♂ = man in steamy room
default-🧖‍♀ = sauna
tts-🧖‍♀ = woman in steamy room
default-🧗 = climber
tts-🧗 = person climbing
default-🧗‍♂ = climber
tts-🧗‍♂ = man climbing
default-🧗‍♀ = climber
tts-🧗‍♀ = woman climbing
default-🤺 = fencer
tts-🤺 = person fencing
default-🏇 = horse
tts-🏇 = horse racing
default-⛷ = ski
tts-⛷ = skier
default-🏂 = ski
tts-🏂 = snowboarder
default-🏌 = ball
tts-🏌 = person golfing
default-🏌‍♂ = golf
tts-🏌‍♂ = man golfing
default-🏌‍♀ = golf
tts-🏌‍♀ = woman golfing
default-🏄 = person surfing
tts-🏄 = person surfing
default-🏄‍♂ = man
tts-🏄‍♂ = man surfing
default-🏄‍♀ = surfing
tts-🏄‍♀ = woman surfing
default-🚣 = boat
tts-🚣 = person rowing boat
default-🚣‍♂ = boat
tts-🚣‍♂ = man rowing boat
default-🚣‍♀ = boat
tts-🚣‍♀ = woman rowing boat
default-🏊 = person swimming
tts-🏊 = person swimming
default-🏊‍♂ = man
tts-🏊‍♂ = man swimming
default-🏊‍♀ = swim
tts-🏊‍♀ = woman swimming
default-⛹ = ball
tts-⛹ = person bouncing ball
default-⛹‍♂ = ball
tts-⛹‍♂ = man bouncing ball
default-⛹‍♀ = ball
tts-⛹‍♀ = woman bouncing ball
default-🏋 = lifter
tts-🏋 = person lifting weights
default-🏋‍♂ = man
tts-🏋‍♂ = man lifting weights
default-🏋‍♀ = weight lifter
tts-🏋‍♀ = woman lifting weights
default-🚴 = bicycle
tts-🚴 = person biking
default-🚴‍♂ = bicycle
tts-🚴‍♂ = man biking
default-🚴‍♀ = bicycle
tts-🚴‍♀ = woman biking
default-🚵 = bicycle
tts-🚵 = person mountain biking
default-🚵‍♂ = bicycle
tts-🚵‍♂ = man mountain biking
default-🚵‍♀ = bicycle
tts-🚵‍♀ = woman mountain biking
default-🤸 = cartwheel
tts-🤸 = person cartwheeling
default-🤸‍♂ = cartwheel
tts-🤸‍♂ = man cartwheeling
default-🤸‍♀ = cartwheel
tts-🤸‍♀ = woman cartwheeling
default-🤼 = people wrestling
tts-🤼 = people wrestling
default-🤼‍♂ = men
tts-🤼‍♂ = men wrestling
default-🤼‍♀ = women
tts-🤼‍♀ = women wrestling
default-🤽 = person playing water polo
tts-🤽 = person playing water polo
default-🤽‍♂ = man
tts-🤽‍♂ = man playing water polo
default-🤽‍♀ = water polo
tts-🤽‍♀ = woman playing water polo
default-🤾 = ball
tts-🤾 = person playing handball
default-🤾‍♂ = handball
tts-🤾‍♂ = man playing handball
default-🤾‍♀ = handball
tts-🤾‍♀ = woman playing handball
default-🤹 = balance
tts-🤹 = person juggling
default-🤹‍♂ = juggling
tts-🤹‍♂ = man juggling
default-🤹‍♀ = juggling
tts-🤹‍♀ = woman juggling
default-🧘 = meditation
tts-🧘 = person in lotus position
default-🧘‍♂ = man in lotus position
tts-🧘‍♂ = man in lotus position
default-🧘‍♀ = meditation
tts-🧘‍♀ = woman in lotus position
default-🛀 = bath
tts-🛀 = person taking bath
default-🛌 = good night
tts-🛌 = person in bed
default-🧑‍🤝‍🧑 = couple
tts-🧑‍🤝‍🧑 = people holding hands
default-👭 = couple
tts-👭 = women holding hands
default-👫 = couple
tts-👫 = woman and man holding hands
default-👬 = couple
tts-👬 = men holding hands
default-💏 = couple
tts-💏 = kiss
default-💑 = couple
tts-💑 = couple with heart
default-🗣 = face
tts-🗣 = speaking head
default-👤 = bust
tts-👤 = bust in silhouette
default-👥 = bust
tts-👥 = busts in silhouette
default-🫂 = goodbye
tts-🫂 = people hugging
default-👪 = family
tts-👪 = family
default-🧑‍🧑‍🧒 = family: adult, adult, child
tts-🧑‍🧑‍🧒 = family: adult, adult, child
default-🧑‍🧑‍🧒‍🧒 = family: adult, adult, child, child
tts-🧑‍🧑‍🧒‍🧒 = family: adult, adult, child, child
default-🧑‍🧒 = family: adult, child
tts-🧑‍🧒 = family: adult, child
default-🧑‍🧒‍🧒 = family: adult, child, child
tts-🧑‍🧒‍🧒 = family: adult, child, child
default-👣 = clothing
tts-👣 = footprints
default-🦰 = ginger
tts-🦰 = red hair
default-🦱 = afro
tts-🦱 = curly hair
default-🦳 = gray
tts-🦳 = white hair
default-🦲 = bald
tts-🦲 = bald
default-🐵 = face
tts-🐵 = monkey face
default-🐒 = monkey
tts-🐒 = monkey
default-🦍 = gorilla
tts-🦍 = gorilla
default-🦧 = ape
tts-🦧 = orangutan
default-🐶 = dog
tts-🐶 = dog face
default-🐕 = dog
tts-🐕 = dog
default-🦮 = accessibility
tts-🦮 = guide dog
default-🐕‍🦺 = accessibility
tts-🐕‍🦺 = service dog
default-🐩 = dog
tts-🐩 = poodle
default-🐺 = face
tts-🐺 = wolf
default-🦊 = face
tts-🦊 = fox
default-🦝 = curious
tts-🦝 = raccoon
default-🐱 = cat
tts-🐱 = cat face
default-🐈 = cat
tts-🐈 = cat
default-🐈‍⬛ = black
tts-🐈‍⬛ = black cat
default-🦁 = face
tts-🦁 = lion
default-🐯 = face
tts-🐯 = tiger face
default-🐅 = tiger
tts-🐅 = tiger
default-🐆 = leopard
tts-🐆 = leopard
default-🐴 = face
tts-🐴 = horse face
default-🫎 = animal
tts-🫎 = moose
default-🫏 = animal
tts-🫏 = donkey
default-🐎 = equestrian
tts-🐎 = horse
default-🦄 = face
tts-🦄 = unicorn
default-🦓 = stripe
tts-🦓 = zebra
default-🦌 = deer
tts-🦌 = deer
default-🦬 = bison
tts-🦬 = bison
default-🐮 = cow
tts-🐮 = cow face
default-🐂 = bull
tts-🐂 = ox
default-🐃 = buffalo
tts-🐃 = water buffalo
default-🐄 = cow
tts-🐄 = cow
default-🐷 = face
tts-🐷 = pig face
default-🐖 = pig
tts-🐖 = pig
default-🐗 = boar
tts-🐗 = boar
default-🐽 = face
tts-🐽 = pig nose
default-🐏 = Aries
tts-🐏 = ram
default-🐑 = ewe
tts-🐑 = ewe
default-🐐 = Capricorn
tts-🐐 = goat
default-🐪 = camel
tts-🐪 = camel
default-🐫 = bactrian
tts-🐫 = two-hump camel
default-🦙 = alpaca
tts-🦙 = llama
default-🦒 = giraffe
tts-🦒 = giraffe
default-🐘 = elephant
tts-🐘 = elephant
default-🦣 = extinction
tts-🦣 = mammoth
default-🦏 = rhinoceros
tts-🦏 = rhinoceros
default-🦛 = hippo
tts-🦛 = hippopotamus
default-🐭 = face
tts-🐭 = mouse face
default-🐁 = mouse
tts-🐁 = mouse
default-🐀 = rat
tts-🐀 = rat
default-🐹 = face
tts-🐹 = hamster
default-🐰 = bunny
tts-🐰 = rabbit face
default-🐇 = bunny
tts-🐇 = rabbit
default-🐿 = chipmunk
tts-🐿 = chipmunk
default-🦫 = beaver
tts-🦫 = beaver
default-🦔 = hedgehog
tts-🦔 = hedgehog
default-🦇 = bat
tts-🦇 = bat
default-🐻 = bear
tts-🐻 = bear
default-🐻‍❄ = arctic
tts-🐻‍❄ = polar bear
default-🐨 = face
tts-🐨 = koala
default-🐼 = face
tts-🐼 = panda
default-🦥 = lazy
tts-🦥 = sloth
default-🦦 = fishing
tts-🦦 = otter
default-🦨 = skunk
tts-🦨 = skunk
default-🦘 = joey
tts-🦘 = kangaroo
default-🦡 = badger
tts-🦡 = badger
default-🐾 = feet
tts-🐾 = paw prints
default-🦃 = bird
tts-🦃 = turkey
default-🐔 = bird
tts-🐔 = chicken
default-🐓 = bird
tts-🐓 = rooster
default-🐣 = baby
tts-🐣 = hatching chick
default-🐤 = baby
tts-🐤 = baby chick
default-🐥 = baby
tts-🐥 = front-facing baby chick
default-🐦 = bird
tts-🐦 = bird
default-🐧 = bird
tts-🐧 = penguin
default-🕊 = bird
tts-🕊 = dove
default-🦅 = bird
tts-🦅 = eagle
default-🦆 = bird
tts-🦆 = duck
default-🦢 = bird
tts-🦢 = swan
default-🦉 = bird
tts-🦉 = owl
default-🦤 = dodo
tts-🦤 = dodo
default-🪶 = bird
tts-🪶 = feather
default-🦩 = flamboyant
tts-🦩 = flamingo
default-🦚 = bird
tts-🦚 = peacock
default-🦜 = bird
tts-🦜 = parrot
default-🪽 = angelic
tts-🪽 = wing
default-🐦‍⬛ = bird
tts-🐦‍⬛ = black bird
default-🪿 = bird
tts-🪿 = goose
default-🐦‍🔥 = fantasy
tts-🐦‍🔥 = phoenix
default-🐸 = face
tts-🐸 = frog
default-🐊 = crocodile
tts-🐊 = crocodile
default-🐢 = terrapin
tts-🐢 = turtle
default-🦎 = lizard
tts-🦎 = lizard
default-🐍 = bearer
tts-🐍 = snake
default-🐲 = dragon
tts-🐲 = dragon face
default-🐉 = dragon
tts-🐉 = dragon
default-🦕 = brachiosaurus
tts-🦕 = sauropod
default-🦖 = T-Rex
tts-🦖 = T-Rex
default-🐳 = face
tts-🐳 = spouting whale
default-🐋 = whale
tts-🐋 = whale
default-🐬 = dolphin
tts-🐬 = dolphin
default-🦭 = sea lion
tts-🦭 = seal
default-🐟 = fish
tts-🐟 = fish
default-🐠 = fish
tts-🐠 = tropical fish
default-🐡 = blowfish
tts-🐡 = blowfish
default-🦈 = fish
tts-🦈 = shark
default-🐙 = octopus
tts-🐙 = octopus
default-🐚 = shell
tts-🐚 = spiral shell
default-🪸 = coral
tts-🪸 = coral
default-🪼 = burn
tts-🪼 = jellyfish
default-🐌 = snail
tts-🐌 = snail
default-🦋 = butterfly
tts-🦋 = butterfly
default-🐛 = bug
tts-🐛 = bug
default-🐜 = ant
tts-🐜 = ant
default-🐝 = bee
tts-🐝 = honeybee
default-🪲 = beetle
tts-🪲 = beetle
default-🐞 = beetle
tts-🐞 = lady beetle
default-🦗 = cricket
tts-🦗 = cricket
default-🪳 = cockroach
tts-🪳 = cockroach
default-🕷 = insect
tts-🕷 = spider
default-🕸 = spider
tts-🕸 = spider web
default-🦂 = Scorpio
tts-🦂 = scorpion
default-🦟 = disease
tts-🦟 = mosquito
default-🪰 = disease
tts-🪰 = fly
default-🪱 = annelid
tts-🪱 = worm
default-🦠 = amoeba
tts-🦠 = microbe
default-💐 = bouquet
tts-💐 = bouquet
default-🌸 = blossom
tts-🌸 = cherry blossom
default-💮 = flower
tts-💮 = white flower
default-🪷 = Buddhism
tts-🪷 = lotus
default-🏵 = plant
tts-🏵 = rosette
default-🌹 = flower
tts-🌹 = rose
default-🥀 = flower
tts-🥀 = wilted flower
default-🌺 = flower
tts-🌺 = hibiscus
default-🌻 = flower
tts-🌻 = sunflower
default-🌼 = blossom
tts-🌼 = blossom
default-🌷 = flower
tts-🌷 = tulip
default-🪻 = bluebonnet
tts-🪻 = hyacinth
default-🌱 = seedling
tts-🌱 = seedling
default-🪴 = boring
tts-🪴 = potted plant
default-🌲 = evergreen tree
tts-🌲 = evergreen tree
default-🌳 = deciduous
tts-🌳 = deciduous tree
default-🌴 = palm
tts-🌴 = palm tree
default-🌵 = cactus
tts-🌵 = cactus
default-🌾 = ear
tts-🌾 = sheaf of rice
default-🌿 = herb
tts-🌿 = herb
default-☘ = plant
tts-☘ = shamrock
default-🍀 = 4
tts-🍀 = four leaf clover
default-🍁 = falling
tts-🍁 = maple leaf
default-🍂 = fallen leaf
tts-🍂 = fallen leaf
default-🍃 = blow
tts-🍃 = leaf fluttering in wind
default-🪹 = empty nest
tts-🪹 = empty nest
default-🪺 = nest with eggs
tts-🪺 = nest with eggs
default-🍄 = mushroom
tts-🍄 = mushroom
default-🍇 = fruit
tts-🍇 = grapes
default-🍈 = fruit
tts-🍈 = melon
default-🍉 = fruit
tts-🍉 = watermelon
default-🍊 = fruit
tts-🍊 = tangerine
default-🍋 = citrus
tts-🍋 = lemon
default-🍋‍🟩 = citrus
tts-🍋‍🟩 = lime
default-🍌 = banana
tts-🍌 = banana
default-🍍 = fruit
tts-🍍 = pineapple
default-🥭 = fruit
tts-🥭 = mango
default-🍎 = apple
tts-🍎 = red apple
default-🍏 = apple
tts-🍏 = green apple
default-🍐 = fruit
tts-🍐 = pear
default-🍑 = fruit
tts-🍑 = peach
default-🍒 = berries
tts-🍒 = cherries
default-🍓 = berry
tts-🍓 = strawberry
default-🫐 = berry
tts-🫐 = blueberries
default-🥝 = food
tts-🥝 = kiwi fruit
default-🍅 = fruit
tts-🍅 = tomato
default-🫒 = food
tts-🫒 = olive
default-🥥 = coconut
tts-🥥 = coconut
default-🥑 = avocado
tts-🥑 = avocado
default-🍆 = aubergine
tts-🍆 = eggplant
default-🥔 = food
tts-🥔 = potato
default-🥕 = carrot
tts-🥕 = carrot
default-🌽 = corn
tts-🌽 = ear of corn
default-🌶 = hot
tts-🌶 = hot pepper
default-🫑 = bell pepper
tts-🫑 = bell pepper
default-🥒 = cucumber
tts-🥒 = cucumber
default-🥬 = bok choy
tts-🥬 = leafy green
default-🥦 = broccoli
tts-🥦 = broccoli
default-🧄 = flavoring
tts-🧄 = garlic
default-🧅 = flavoring
tts-🧅 = onion
default-🥜 = food
tts-🥜 = peanuts
default-🫘 = beans
tts-🫘 = beans
default-🌰 = chestnut
tts-🌰 = chestnut
default-🫚 = beer
tts-🫚 = ginger root
default-🫛 = beans
tts-🫛 = pea pod
default-🍄‍🟫 = brown mushroom
tts-🍄‍🟫 = brown mushroom
default-🍞 = bread
tts-🍞 = bread
default-🥐 = bread
tts-🥐 = croissant
default-🥖 = baguette
tts-🥖 = baguette bread
default-🫓 = arepa
tts-🫓 = flatbread
default-🥨 = pretzel
tts-🥨 = pretzel
default-🥯 = bagel
tts-🥯 = bagel
default-🥞 = breakfast
tts-🥞 = pancakes
default-🧇 = breakfast
tts-🧇 = waffle
default-🧀 = cheese
tts-🧀 = cheese wedge
default-🍖 = bone
tts-🍖 = meat on bone
default-🍗 = bone
tts-🍗 = poultry leg
default-🥩 = chop
tts-🥩 = cut of meat
default-🥓 = bacon
tts-🥓 = bacon
default-🍔 = burger
tts-🍔 = hamburger
default-🍟 = french
tts-🍟 = french fries
default-🍕 = cheese
tts-🍕 = pizza
default-🌭 = frankfurter
tts-🌭 = hot dog
default-🥪 = bread
tts-🥪 = sandwich
default-🌮 = mexican
tts-🌮 = taco
default-🌯 = burrito
tts-🌯 = burrito
default-🫔 = mexican
tts-🫔 = tamale
default-🥙 = falafel
tts-🥙 = stuffed flatbread
default-🧆 = chickpea
tts-🧆 = falafel
default-🥚 = breakfast
tts-🥚 = egg
default-🍳 = breakfast
tts-🍳 = cooking
default-🥘 = casserole
tts-🥘 = shallow pan of food
default-🍲 = pot
tts-🍲 = pot of food
default-🫕 = cheese
tts-🫕 = fondue
default-🥣 = bowl with spoon
tts-🥣 = bowl with spoon
default-🥗 = food
tts-🥗 = green salad
default-🍿 = popcorn
tts-🍿 = popcorn
default-🧈 = butter
tts-🧈 = butter
default-🧂 = condiment
tts-🧂 = salt
default-🥫 = can
tts-🥫 = canned food
default-🍱 = bento
tts-🍱 = bento box
default-🍘 = cracker
tts-🍘 = rice cracker
default-🍙 = ball
tts-🍙 = rice ball
default-🍚 = cooked
tts-🍚 = cooked rice
default-🍛 = curry
tts-🍛 = curry rice
default-🍜 = bowl
tts-🍜 = steaming bowl
default-🍝 = pasta
tts-🍝 = spaghetti
default-🍠 = potato
tts-🍠 = roasted sweet potato
default-🍢 = kebab
tts-🍢 = oden
default-🍣 = sushi
tts-🍣 = sushi
default-🍤 = fried
tts-🍤 = fried shrimp
default-🍥 = cake
tts-🍥 = fish cake with swirl
default-🥮 = autumn
tts-🥮 = moon cake
default-🍡 = dango
tts-🍡 = dango
default-🥟 = dumpling
tts-🥟 = dumpling
default-🥠 = fortune cookie
tts-🥠 = fortune cookie
default-🥡 = oyster pail
tts-🥡 = takeout box
default-🦀 = Cancer
tts-🦀 = crab
default-🦞 = bisque
tts-🦞 = lobster
default-🦐 = food
tts-🦐 = shrimp
default-🦑 = food
tts-🦑 = squid
default-🦪 = diving
tts-🦪 = oyster
default-🍦 = cream
tts-🍦 = soft ice cream
default-🍧 = dessert
tts-🍧 = shaved ice
default-🍨 = cream
tts-🍨 = ice cream
default-🍩 = breakfast
tts-🍩 = doughnut
default-🍪 = cookie
tts-🍪 = cookie
default-🎂 = birthday
tts-🎂 = birthday cake
default-🍰 = cake
tts-🍰 = shortcake
default-🧁 = bakery
tts-🧁 = cupcake
default-🥧 = filling
tts-🥧 = pie
default-🍫 = bar
tts-🍫 = chocolate bar
default-🍬 = candy
tts-🍬 = candy
default-🍭 = candy
tts-🍭 = lollipop
default-🍮 = custard
tts-🍮 = custard
default-🍯 = honey
tts-🍯 = honey pot
default-🍼 = baby
tts-🍼 = baby bottle
default-🥛 = drink
tts-🥛 = glass of milk
default-☕ = beverage
tts-☕ = hot beverage
default-🫖 = drink
tts-🫖 = teapot
default-🍵 = beverage
tts-🍵 = teacup without handle
default-🍶 = bar
tts-🍶 = sake
default-🍾 = bar
tts-🍾 = bottle with popping cork
default-🍷 = bar
tts-🍷 = wine glass
default-🍸 = bar
tts-🍸 = cocktail glass
default-🍹 = bar
tts-🍹 = tropical drink
default-🍺 = bar
tts-🍺 = beer mug
default-🍻 = bar
tts-🍻 = clinking beer mugs
default-🥂 = celebrate
tts-🥂 = clinking glasses
default-🥃 = glass
tts-🥃 = tumbler glass
default-🫗 = drink
tts-🫗 = pouring liquid
default-🥤 = cup with straw
tts-🥤 = cup with straw
default-🧋 = bubble
tts-🧋 = bubble tea
default-🧃 = beverage
tts-🧃 = beverage box
default-🧉 = drink
tts-🧉 = mate
default-🧊 = cold
tts-🧊 = ice
default-🥢 = chopsticks
tts-🥢 = chopsticks
default-🍽 = cooking
tts-🍽 = fork and knife with plate
default-🍴 = cooking
tts-🍴 = fork and knife
default-🥄 = spoon
tts-🥄 = spoon
default-🔪 = cooking
tts-🔪 = kitchen knife
default-🫙 = condiment
tts-🫙 = jar
default-🏺 = amphora
tts-🏺 = amphora
default-🌍 = Africa
tts-🌍 = globe showing Europe-Africa
default-🌎 = Americas
tts-🌎 = globe showing Americas
default-🌏 = Asia
tts-🌏 = globe showing Asia-Australia
default-🌐 = earth
tts-🌐 = globe with meridians
default-🗺 = map
tts-🗺 = world map
default-🗾 = Japan
tts-🗾 = map of Japan
default-🧭 = compass
tts-🧭 = compass
default-🏔 = cold
tts-🏔 = snow-capped mountain
default-⛰ = mountain
tts-⛰ = mountain
default-🌋 = eruption
tts-🌋 = volcano
default-🗻 = fuji
tts-🗻 = mount fuji
default-🏕 = camping
tts-🏕 = camping
default-🏖 = beach
tts-🏖 = beach with umbrella
default-🏜 = desert
tts-🏜 = desert
default-🏝 = desert
tts-🏝 = desert island
default-🏞 = national park
tts-🏞 = national park
default-🏟 = stadium
tts-🏟 = stadium
default-🏛 = classical
tts-🏛 = classical building
default-🏗 = building construction
tts-🏗 = building construction
default-🧱 = brick
tts-🧱 = brick
default-🪨 = boulder
tts-🪨 = rock
default-🪵 = log
tts-🪵 = wood
default-🛖 = house
tts-🛖 = hut
default-🏘 = houses
tts-🏘 = houses
default-🏚 = derelict
tts-🏚 = derelict house
default-🏠 = home
tts-🏠 = house
default-🏡 = garden
tts-🏡 = house with garden
default-🏢 = building
tts-🏢 = office building
default-🏣 = Japanese
tts-🏣 = Japanese post office
default-🏤 = European
tts-🏤 = post office
default-🏥 = doctor
tts-🏥 = hospital
default-🏦 = bank
tts-🏦 = bank
default-🏨 = building
tts-🏨 = hotel
default-🏩 = hotel
tts-🏩 = love hotel
default-🏪 = convenience
tts-🏪 = convenience store
default-🏫 = building
tts-🏫 = school
default-🏬 = department
tts-🏬 = department store
default-🏭 = building
tts-🏭 = factory
default-🏯 = castle
tts-🏯 = Japanese castle
default-🏰 = castle
tts-🏰 = castle
default-💒 = chapel
tts-💒 = wedding
default-🗼 = Tokyo
tts-🗼 = Tokyo tower
default-🗽 = liberty
tts-🗽 = Statue of Liberty
default-⛪ = Christian
tts-⛪ = church
default-🕌 = islam
tts-🕌 = mosque
default-🛕 = hindu
tts-🛕 = hindu temple
default-🕍 = Jew
tts-🕍 = synagogue
default-⛩ = religion
tts-⛩ = shinto shrine
default-🕋 = islam
tts-🕋 = kaaba
default-⛲ = fountain
tts-⛲ = fountain
default-⛺ = camping
tts-⛺ = tent
default-🌁 = fog
tts-🌁 = foggy
default-🌃 = night
tts-🌃 = night with stars
default-🏙 = city
tts-🏙 = cityscape
default-🌄 = morning
tts-🌄 = sunrise over mountains
default-🌅 = morning
tts-🌅 = sunrise
default-🌆 = city
tts-🌆 = cityscape at dusk
default-🌇 = dusk
tts-🌇 = sunset
default-🌉 = bridge
tts-🌉 = bridge at night
default-♨ = hot
tts-♨ = hot springs
default-🎠 = carousel
tts-🎠 = carousel horse
default-🛝 = amusement park
tts-🛝 = playground slide
default-🎡 = amusement park
tts-🎡 = ferris wheel
default-🎢 = amusement park
tts-🎢 = roller coaster
default-💈 = barber
tts-💈 = barber pole
default-🎪 = circus
tts-🎪 = circus tent
default-🚂 = engine
tts-🚂 = locomotive
default-🚃 = car
tts-🚃 = railway car
default-🚄 = high-speed train
tts-🚄 = high-speed train
default-🚅 = bullet
tts-🚅 = bullet train
default-🚆 = railway
tts-🚆 = train
default-🚇 = metro
tts-🚇 = metro
default-🚈 = light rail
tts-🚈 = light rail
default-🚉 = railway
tts-🚉 = station
default-🚊 = tram
tts-🚊 = tram
default-🚝 = monorail
tts-🚝 = monorail
default-🚞 = car
tts-🚞 = mountain railway
default-🚋 = car
tts-🚋 = tram car
default-🚌 = bus
tts-🚌 = bus
default-🚍 = bus
tts-🚍 = oncoming bus
default-🚎 = bus
tts-🚎 = trolleybus
default-🚐 = bus
tts-🚐 = minibus
default-🚑 = ambulance
tts-🚑 = ambulance
default-🚒 = engine
tts-🚒 = fire engine
default-🚓 = car
tts-🚓 = police car
default-🚔 = car
tts-🚔 = oncoming police car
default-🚕 = taxi
tts-🚕 = taxi
default-🚖 = oncoming
tts-🚖 = oncoming taxi
default-🚗 = automobile
tts-🚗 = automobile
default-🚘 = automobile
tts-🚘 = oncoming automobile
default-🚙 = recreational
tts-🚙 = sport utility vehicle
default-🛻 = pick-up
tts-🛻 = pickup truck
default-🚚 = delivery
tts-🚚 = delivery truck
default-🚛 = articulated lorry
tts-🚛 = articulated lorry
default-🚜 = tractor
tts-🚜 = tractor
default-🏎 = car
tts-🏎 = racing car
default-🏍 = motorcycle
tts-🏍 = motorcycle
default-🛵 = motor
tts-🛵 = motor scooter
default-🦽 = accessibility
tts-🦽 = manual wheelchair
default-🦼 = accessibility
tts-🦼 = motorized wheelchair
default-🛺 = auto rickshaw
tts-🛺 = auto rickshaw
default-🚲 = bicycle
tts-🚲 = bicycle
default-🛴 = kick
tts-🛴 = kick scooter
default-🛹 = board
tts-🛹 = skateboard
default-🛼 = roller
tts-🛼 = roller skate
default-🚏 = bus
tts-🚏 = bus stop
default-🛣 = highway
tts-🛣 = motorway
default-🛤 = railway
tts-🛤 = railway track
default-🛢 = drum
tts-🛢 = oil drum
default-⛽ = diesel
tts-⛽ = fuel pump
default-🛞 = circle
tts-🛞 = wheel
default-🚨 = beacon
tts-🚨 = police car light
default-🚥 = horizontal traffic light
tts-🚥 = horizontal traffic light
default-🚦 = light
tts-🚦 = vertical traffic light
default-🛑 = octagonal
tts-🛑 = stop sign
default-🚧 = barrier
tts-🚧 = construction
default-⚓ = anchor
tts-⚓ = anchor
default-🛟 = float
tts-🛟 = ring buoy
default-⛵ = boat
tts-⛵ = sailboat
default-🛶 = boat
tts-🛶 = canoe
default-🚤 = boat
tts-🚤 = speedboat
default-🛳 = passenger
tts-🛳 = passenger ship
default-⛴ = boat
tts-⛴ = ferry
default-🛥 = boat
tts-🛥 = motor boat
default-🚢 = boat
tts-🚢 = ship
default-✈ = aeroplane
tts-✈ = airplane
default-🛩 = aeroplane
tts-🛩 = small airplane
default-🛫 = aeroplane
tts-🛫 = airplane departure
default-🛬 = aeroplane
tts-🛬 = airplane arrival
default-🪂 = hang-glide
tts-🪂 = parachute
default-💺 = chair
tts-💺 = seat
default-🚁 = helicopter
tts-🚁 = helicopter
default-🚟 = railway
tts-🚟 = suspension railway
default-🚠 = cable
tts-🚠 = mountain cableway
default-🚡 = aerial
tts-🚡 = aerial tramway
default-🛰 = satellite
tts-🛰 = satellite
default-🚀 = rocket
tts-🚀 = rocket
default-🛸 = flying saucer
tts-🛸 = flying saucer
default-🛎 = bell
tts-🛎 = bellhop bell
default-🧳 = luggage
tts-🧳 = luggage
default-⌛ = hourglass done
tts-⌛ = hourglass done
default-⏳ = hourglass
tts-⏳ = hourglass not done
default-⌚ = clock
tts-⌚ = watch
default-⏰ = alarm
tts-⏰ = alarm clock
default-⏱ = clock
tts-⏱ = stopwatch
default-⏲ = clock
tts-⏲ = timer clock
default-🕰 = clock
tts-🕰 = mantelpiece clock
default-🕛 = 00
tts-🕛 = twelve o’clock
default-🕧 = 12
tts-🕧 = twelve-thirty
default-🕐 = 00
tts-🕐 = one o’clock
default-🕜 = 1
tts-🕜 = one-thirty
default-🕑 = 00
tts-🕑 = two o’clock
default-🕝 = 2
tts-🕝 = two-thirty
default-🕒 = 00
tts-🕒 = three o’clock
default-🕞 = 3
tts-🕞 = three-thirty
default-🕓 = 00
tts-🕓 = four o’clock
default-🕟 = 4
tts-🕟 = four-thirty
default-🕔 = 00
tts-🕔 = five o’clock
default-🕠 = 5
tts-🕠 = five-thirty
default-🕕 = 00
tts-🕕 = six o’clock
default-🕡 = 6
tts-🕡 = six-thirty
default-🕖 = 00
tts-🕖 = seven o’clock
default-🕢 = 7
tts-🕢 = seven-thirty
default-🕗 = 00
tts-🕗 = eight o’clock
default-🕣 = 8
tts-🕣 = eight-thirty
default-🕘 = 00
tts-🕘 = nine o’clock
default-🕤 = 9
tts-🕤 = nine-thirty
default-🕙 = 00
tts-🕙 = ten o’clock
default-🕥 = 10
tts-🕥 = ten-thirty
default-🕚 = 00
tts-🕚 = eleven o’clock
default-🕦 = 11
tts-🕦 = eleven-thirty
default-🌑 = dark
tts-🌑 = new moon
default-🌒 = crescent
tts-🌒 = waxing crescent moon
default-🌓 = first quarter moon
tts-🌓 = first quarter moon
default-🌔 = gibbous
tts-🌔 = waxing gibbous moon
default-🌕 = full
tts-🌕 = full moon
default-🌖 = gibbous
tts-🌖 = waning gibbous moon
default-🌗 = last quarter moon
tts-🌗 = last quarter moon
default-🌘 = crescent
tts-🌘 = waning crescent moon
default-🌙 = crescent
tts-🌙 = crescent moon
default-🌚 = face
tts-🌚 = new moon face
default-🌛 = face
tts-🌛 = first quarter moon face
default-🌜 = face
tts-🌜 = last quarter moon face
default-🌡 = thermometer
tts-🌡 = thermometer
default-☀ = bright
tts-☀ = sun
default-🌝 = bright
tts-🌝 = full moon face
default-🌞 = bright
tts-🌞 = sun with face
default-🪐 = ringed planet
tts-🪐 = ringed planet
default-⭐ = star
tts-⭐ = star
default-🌟 = glittery
tts-🌟 = glowing star
default-🌠 = falling
tts-🌠 = shooting star
default-🌌 = milky way
tts-🌌 = milky way
default-☁ = cloud
tts-☁ = cloud
default-⛅ = cloud
tts-⛅ = sun behind cloud
default-⛈ = cloud
tts-⛈ = cloud with lightning and rain
default-🌤 = cloud
tts-🌤 = sun behind small cloud
default-🌥 = cloud
tts-🌥 = sun behind large cloud
default-🌦 = cloud
tts-🌦 = sun behind rain cloud
default-🌧 = cloud
tts-🌧 = cloud with rain
default-🌨 = cloud
tts-🌨 = cloud with snow
default-🌩 = cloud
tts-🌩 = cloud with lightning
default-🌪 = cloud
tts-🌪 = tornado
default-🌫 = cloud
tts-🌫 = fog
default-🌬 = blow
tts-🌬 = wind face
default-🌀 = cyclone
tts-🌀 = cyclone
default-🌈 = rain
tts-🌈 = rainbow
default-🌂 = closed umbrella
tts-🌂 = closed umbrella
default-☂ = clothing
tts-☂ = umbrella
default-☔ = clothing
tts-☔ = umbrella with rain drops
default-⛱ = rain
tts-⛱ = umbrella on ground
default-⚡ = danger
tts-⚡ = high voltage
default-❄ = cold
tts-❄ = snowflake
default-☃ = cold
tts-☃ = snowman
default-⛄ = cold
tts-⛄ = snowman without snow
default-☄ = comet
tts-☄ = comet
default-🔥 = fire
tts-🔥 = fire
default-💧 = cold
tts-💧 = droplet
default-🌊 = ocean
tts-🌊 = water wave
default-🎃 = celebration
tts-🎃 = jack-o-lantern
default-🎄 = celebration
tts-🎄 = Christmas tree
default-🎆 = celebration
tts-🎆 = fireworks
default-🎇 = celebration
tts-🎇 = sparkler
default-🧨 = dynamite
tts-🧨 = firecracker
default-✨ = *
tts-✨ = sparkles
default-🎈 = balloon
tts-🎈 = balloon
default-🎉 = celebration
tts-🎉 = party popper
default-🎊 = ball
tts-🎊 = confetti ball
default-🎋 = banner
tts-🎋 = tanabata tree
default-🎍 = bamboo
tts-🎍 = pine decoration
default-🎎 = celebration
tts-🎎 = Japanese dolls
default-🎏 = carp
tts-🎏 = carp streamer
default-🎐 = bell
tts-🎐 = wind chime
default-🎑 = celebration
tts-🎑 = moon viewing ceremony
default-🧧 = gift
tts-🧧 = red envelope
default-🎀 = celebration
tts-🎀 = ribbon
default-🎁 = box
tts-🎁 = wrapped gift
default-🎗 = celebration
tts-🎗 = reminder ribbon
default-🎟 = admission
tts-🎟 = admission tickets
default-🎫 = admission
tts-🎫 = ticket
default-🎖 = celebration
tts-🎖 = military medal
default-🏆 = prize
tts-🏆 = trophy
default-🏅 = medal
tts-🏅 = sports medal
default-🥇 = 1st place medal
tts-🥇 = 1st place medal
default-🥈 = 2nd place medal
tts-🥈 = 2nd place medal
default-🥉 = 3rd place medal
tts-🥉 = 3rd place medal
default-⚽ = ball
tts-⚽ = soccer ball
default-⚾ = ball
tts-⚾ = baseball
default-🥎 = ball
tts-🥎 = softball
default-🏀 = ball
tts-🏀 = basketball
default-🏐 = ball
tts-🏐 = volleyball
default-🏈 = american
tts-🏈 = american football
default-🏉 = ball
tts-🏉 = rugby football
default-🎾 = ball
tts-🎾 = tennis
default-🥏 = flying disc
tts-🥏 = flying disc
default-🎳 = ball
tts-🎳 = bowling
default-🏏 = ball
tts-🏏 = cricket game
default-🏑 = ball
tts-🏑 = field hockey
default-🏒 = game
tts-🏒 = ice hockey
default-🥍 = ball
tts-🥍 = lacrosse
default-🏓 = ball
tts-🏓 = ping pong
default-🏸 = badminton
tts-🏸 = badminton
default-🥊 = boxing
tts-🥊 = boxing glove
default-🥋 = judo
tts-🥋 = martial arts uniform
default-🥅 = goal
tts-🥅 = goal net
default-⛳ = flag in hole
tts-⛳ = flag in hole
default-⛸ = ice
tts-⛸ = ice skate
default-🎣 = fish
tts-🎣 = fishing pole
default-🤿 = diving
tts-🤿 = diving mask
default-🎽 = athletics
tts-🎽 = running shirt
default-🎿 = ski
tts-🎿 = skis
default-🛷 = sled
tts-🛷 = sled
default-🥌 = curling stone
tts-🥌 = curling stone
default-🎯 = bullseye
tts-🎯 = bullseye
default-🪀 = fluctuate
tts-🪀 = yo-yo
default-🪁 = fly
tts-🪁 = kite
default-🔫 = gun
tts-🔫 = water pistol
default-🎱 = 8
tts-🎱 = pool 8 ball
default-🔮 = ball
tts-🔮 = crystal ball
default-🪄 = magic
tts-🪄 = magic wand
default-🎮 = controller
tts-🎮 = video game
default-🕹 = game
tts-🕹 = joystick
default-🎰 = game
tts-🎰 = slot machine
default-🎲 = dice
tts-🎲 = game die
default-🧩 = clue
tts-🧩 = puzzle piece
default-🧸 = plaything
tts-🧸 = teddy bear
default-🪅 = celebration
tts-🪅 = piñata
default-🪩 = dance
tts-🪩 = mirror ball
default-🪆 = doll
tts-🪆 = nesting dolls
default-♠ = card
tts-♠ = spade suit
default-♥ = card
tts-♥ = heart suit
default-♦ = card
tts-♦ = diamond suit
default-♣ = card
tts-♣ = club suit
default-♟ = chess
tts-♟ = chess pawn
default-🃏 = card
tts-🃏 = joker
default-🀄 = game
tts-🀄 = mahjong red dragon
default-🎴 = card
tts-🎴 = flower playing cards
default-🎭 = art
tts-🎭 = performing arts
default-🖼 = art
tts-🖼 = framed picture
default-🎨 = art
tts-🎨 = artist palette
default-🧵 = needle
tts-🧵 = thread
default-🪡 = embroidery
tts-🪡 = sewing needle
default-🧶 = ball
tts-🧶 = yarn
default-🪢 = knot
tts-🪢 = knot
default-👓 = clothing
tts-👓 = glasses
default-🕶 = dark
tts-🕶 = sunglasses
default-🥽 = eye protection
tts-🥽 = goggles
default-🥼 = doctor
tts-🥼 = lab coat
default-🦺 = emergency
tts-🦺 = safety vest
default-👔 = clothing
tts-👔 = necktie
default-👕 = clothing
tts-👕 = t-shirt
default-👖 = clothing
tts-👖 = jeans
default-🧣 = neck
tts-🧣 = scarf
default-🧤 = gloves
tts-🧤 = gloves
default-🧥 = coat
tts-🧥 = coat
default-🧦 = socks
tts-🧦 = socks
default-👗 = clothing
tts-👗 = dress
default-👘 = clothing
tts-👘 = kimono
default-🥻 = clothing
tts-🥻 = sari
default-🩱 = bathing suit
tts-🩱 = one-piece swimsuit
default-🩲 = bathing suit
tts-🩲 = briefs
default-🩳 = bathing suit
tts-🩳 = shorts
default-👙 = bikini
tts-👙 = bikini
default-👚 = clothing
tts-👚 = woman’s clothes
default-🪭 = cooling
tts-🪭 = folding hand fan
default-👛 = clothing
tts-👛 = purse
default-👜 = bag
tts-👜 = handbag
default-👝 = bag
tts-👝 = clutch bag
default-🛍 = bag
tts-🛍 = shopping bags
default-🎒 = backpack
tts-🎒 = backpack
default-🩴 = beach sandals
tts-🩴 = thong sandal
default-👞 = clothing
tts-👞 = man’s shoe
default-👟 = athletic
tts-👟 = running shoe
default-🥾 = backpacking
tts-🥾 = hiking boot
default-🥿 = ballet flat
tts-🥿 = flat shoe
default-👠 = clothing
tts-👠 = high-heeled shoe
default-👡 = clothing
tts-👡 = woman’s sandal
default-🩰 = ballet
tts-🩰 = ballet shoes
default-👢 = boot
tts-👢 = woman’s boot
default-🪮 = Afro
tts-🪮 = hair pick
default-👑 = clothing
tts-👑 = crown
default-👒 = clothing
tts-👒 = woman’s hat
default-🎩 = clothing
tts-🎩 = top hat
default-🎓 = cap
tts-🎓 = graduation cap
default-🧢 = baseball cap
tts-🧢 = billed cap
default-🪖 = army
tts-🪖 = military helmet
default-⛑ = aid
tts-⛑ = rescue worker’s helmet
default-📿 = beads
tts-📿 = prayer beads
default-💄 = cosmetics
tts-💄 = lipstick
default-💍 = diamond
tts-💍 = ring
default-💎 = diamond
tts-💎 = gem stone
default-🔇 = mute
tts-🔇 = muted speaker
default-🔈 = soft
tts-🔈 = speaker low volume
default-🔉 = medium
tts-🔉 = speaker medium volume
default-🔊 = loud
tts-🔊 = speaker high volume
default-📢 = loud
tts-📢 = loudspeaker
default-📣 = cheering
tts-📣 = megaphone
default-📯 = horn
tts-📯 = postal horn
default-🔔 = bell
tts-🔔 = bell
default-🔕 = bell
tts-🔕 = bell with slash
default-🎼 = music
tts-🎼 = musical score
default-🎵 = music
tts-🎵 = musical note
default-🎶 = music
tts-🎶 = musical notes
default-🎙 = mic
tts-🎙 = studio microphone
default-🎚 = level
tts-🎚 = level slider
default-🎛 = control
tts-🎛 = control knobs
default-🎤 = karaoke
tts-🎤 = microphone
default-🎧 = earbud
tts-🎧 = headphone
default-📻 = radio
tts-📻 = radio
default-🎷 = instrument
tts-🎷 = saxophone
default-🪗 = accordion
tts-🪗 = accordion
default-🎸 = guitar
tts-🎸 = guitar
default-🎹 = instrument
tts-🎹 = musical keyboard
default-🎺 = instrument
tts-🎺 = trumpet
default-🎻 = instrument
tts-🎻 = violin
default-🪕 = banjo
tts-🪕 = banjo
default-🥁 = drum
tts-🥁 = drum
default-🪘 = beat
tts-🪘 = long drum
default-🪇 = instrument
tts-🪇 = maracas
default-🪈 = fife
tts-🪈 = flute
default-📱 = cell
tts-📱 = mobile phone
default-📲 = arrow
tts-📲 = mobile phone with arrow
default-☎ = phone
tts-☎ = telephone
default-📞 = phone
tts-📞 = telephone receiver
default-📟 = pager
tts-📟 = pager
default-📠 = fax
tts-📠 = fax machine
default-🔋 = battery
tts-🔋 = battery
default-🪫 = electronic
tts-🪫 = low battery
default-🔌 = electric
tts-🔌 = electric plug
default-💻 = computer
tts-💻 = laptop
default-🖥 = computer
tts-🖥 = desktop computer
default-🖨 = computer
tts-🖨 = printer
default-⌨ = computer
tts-⌨ = keyboard
default-🖱 = computer
tts-🖱 = computer mouse
default-🖲 = computer
tts-🖲 = trackball
default-💽 = computer
tts-💽 = computer disk
default-💾 = computer
tts-💾 = floppy disk
default-💿 = CD
tts-💿 = optical disk
default-📀 = Blu-ray
tts-📀 = dvd
default-🧮 = abacus
tts-🧮 = abacus
default-🎥 = camera
tts-🎥 = movie camera
default-🎞 = cinema
tts-🎞 = film frames
default-📽 = cinema
tts-📽 = film projector
default-🎬 = clapper
tts-🎬 = clapper board
default-📺 = television
tts-📺 = television
default-📷 = camera
tts-📷 = camera
default-📸 = camera
tts-📸 = camera with flash
default-📹 = camera
tts-📹 = video camera
default-📼 = tape
tts-📼 = videocassette
default-🔍 = glass
tts-🔍 = magnifying glass tilted left
default-🔎 = glass
tts-🔎 = magnifying glass tilted right
default-🕯 = candle
tts-🕯 = candle
default-💡 = bulb
tts-💡 = light bulb
default-🔦 = electric
tts-🔦 = flashlight
default-🏮 = bar
tts-🏮 = red paper lantern
default-🪔 = diya
tts-🪔 = diya lamp
default-📔 = book
tts-📔 = notebook with decorative cover
default-📕 = book
tts-📕 = closed book
default-📖 = book
tts-📖 = open book
default-📗 = book
tts-📗 = green book
default-📘 = blue
tts-📘 = blue book
default-📙 = book
tts-📙 = orange book
default-📚 = book
tts-📚 = books
default-📓 = notebook
tts-📓 = notebook
default-📒 = ledger
tts-📒 = ledger
default-📃 = curl
tts-📃 = page with curl
default-📜 = paper
tts-📜 = scroll
default-📄 = document
tts-📄 = page facing up
default-📰 = news
tts-📰 = newspaper
default-🗞 = news
tts-🗞 = rolled-up newspaper
default-📑 = bookmark
tts-📑 = bookmark tabs
default-🔖 = bookmark
tts-🔖 = bookmark
default-🏷 = label
tts-🏷 = label
default-💰 = bag
tts-💰 = money bag
default-🪙 = coin
tts-🪙 = coin
default-💴 = banknote
tts-💴 = yen banknote
default-💵 = banknote
tts-💵 = dollar banknote
default-💶 = banknote
tts-💶 = euro banknote
default-💷 = banknote
tts-💷 = pound banknote
default-💸 = banknote
tts-💸 = money with wings
default-💳 = card
tts-💳 = credit card
default-🧾 = accounting
tts-🧾 = receipt
default-💹 = chart
tts-💹 = chart increasing with yen
default-✉ = email
tts-✉ = envelope
default-📧 = e-mail
tts-📧 = e-mail
default-📨 = e-mail
tts-📨 = incoming envelope
default-📩 = arrow
tts-📩 = envelope with arrow
default-📤 = box
tts-📤 = outbox tray
default-📥 = box
tts-📥 = inbox tray
default-📦 = box
tts-📦 = package
default-📫 = closed
tts-📫 = closed mailbox with raised flag
default-📪 = closed
tts-📪 = closed mailbox with lowered flag
default-📬 = mail
tts-📬 = open mailbox with raised flag
default-📭 = lowered
tts-📭 = open mailbox with lowered flag
default-📮 = mail
tts-📮 = postbox
default-🗳 = ballot
tts-🗳 = ballot box with ballot
default-✏ = pencil
tts-✏ = pencil
default-✒ = black nib
tts-✒ = black nib
default-🖋 = fountain
tts-🖋 = fountain pen
default-🖊 = ballpoint
tts-🖊 = pen
default-🖌 = paintbrush
tts-🖌 = paintbrush
default-🖍 = crayon
tts-🖍 = crayon
default-📝 = memo
tts-📝 = memo
default-💼 = briefcase
tts-💼 = briefcase
default-📁 = file
tts-📁 = file folder
default-📂 = file
tts-📂 = open file folder
default-🗂 = card
tts-🗂 = card index dividers
default-📅 = calendar
tts-📅 = calendar
default-📆 = calendar
tts-📆 = tear-off calendar
default-🗒 = note
tts-🗒 = spiral notepad
default-🗓 = calendar
tts-🗓 = spiral calendar
default-📇 = card
tts-📇 = card index
default-📈 = chart
tts-📈 = chart increasing
default-📉 = chart
tts-📉 = chart decreasing
default-📊 = bar
tts-📊 = bar chart
default-📋 = clipboard
tts-📋 = clipboard
default-📌 = pin
tts-📌 = pushpin
default-📍 = pin
tts-📍 = round pushpin
default-📎 = paperclip
tts-📎 = paperclip
default-🖇 = link
tts-🖇 = linked paperclips
default-📏 = ruler
tts-📏 = straight ruler
default-📐 = ruler
tts-📐 = triangular ruler
default-✂ = cutting
tts-✂ = scissors
default-🗃 = box
tts-🗃 = card file box
default-🗄 = cabinet
tts-🗄 = file cabinet
default-🗑 = wastebasket
tts-🗑 = wastebasket
default-🔒 = closed
tts-🔒 = locked
default-🔓 = lock
tts-🔓 = unlocked
default-🔏 = ink
tts-🔏 = locked with pen
default-🔐 = closed
tts-🔐 = locked with key
default-🔑 = key
tts-🔑 = key
default-🗝 = clue
tts-🗝 = old key
default-🔨 = hammer
tts-🔨 = hammer
default-🪓 = axe
tts-🪓 = axe
default-⛏ = mining
tts-⛏ = pick
default-⚒ = hammer
tts-⚒ = hammer and pick
default-🛠 = hammer
tts-🛠 = hammer and wrench
default-🗡 = dagger
tts-🗡 = dagger
default-⚔ = crossed
tts-⚔ = crossed swords
default-💣 = bomb
tts-💣 = bomb
default-🪃 = boomerang
tts-🪃 = boomerang
default-🏹 = archer
tts-🏹 = bow and arrow
default-🛡 = shield
tts-🛡 = shield
default-🪚 = carpenter
tts-🪚 = carpentry saw
default-🔧 = spanner
tts-🔧 = wrench
default-🪛 = screw
tts-🪛 = screwdriver
default-🔩 = bolt
tts-🔩 = nut and bolt
default-⚙ = cog
tts-⚙ = gear
default-🗜 = clamp
tts-🗜 = clamp
default-⚖ = balance
tts-⚖ = balance scale
default-🦯 = accessibility
tts-🦯 = white cane
default-🔗 = link
tts-🔗 = link
default-⛓‍💥 = break
tts-⛓‍💥 = broken chain
default-⛓ = chain
tts-⛓ = chains
default-🪝 = catch
tts-🪝 = hook
default-🧰 = chest
tts-🧰 = toolbox
default-🧲 = attraction
tts-🧲 = magnet
default-🪜 = climb
tts-🪜 = ladder
default-⚗ = alembic
tts-⚗ = alembic
default-🧪 = chemist
tts-🧪 = test tube
default-🧫 = bacteria
tts-🧫 = petri dish
default-🧬 = biologist
tts-🧬 = dna
default-🔬 = microscope
tts-🔬 = microscope
default-🔭 = science
tts-🔭 = telescope
default-📡 = antenna
tts-📡 = satellite antenna
default-💉 = medicine
tts-💉 = syringe
default-🩸 = bleed
tts-🩸 = drop of blood
default-💊 = doctor
tts-💊 = pill
default-🩹 = adhesive bandage
tts-🩹 = adhesive bandage
default-🩼 = cane
tts-🩼 = crutch
default-🩺 = doctor
tts-🩺 = stethoscope
default-🩻 = bones
tts-🩻 = x-ray
default-🚪 = door
tts-🚪 = door
default-🛗 = accessibility
tts-🛗 = elevator
default-🪞 = mirror
tts-🪞 = mirror
default-🪟 = frame
tts-🪟 = window
default-🛏 = bed
tts-🛏 = bed
default-🛋 = couch
tts-🛋 = couch and lamp
default-🪑 = chair
tts-🪑 = chair
default-🚽 = toilet
tts-🚽 = toilet
default-🪠 = force cup
tts-🪠 = plunger
default-🚿 = shower
tts-🚿 = shower
default-🛁 = bath
tts-🛁 = bathtub
default-🪤 = bait
tts-🪤 = mouse trap
default-🪒 = razor
tts-🪒 = razor
default-🧴 = lotion
tts-🧴 = lotion bottle
default-🧷 = diaper
tts-🧷 = safety pin
default-🧹 = broom
tts-🧹 = broom
default-🧺 = basket
tts-🧺 = basket
default-🧻 = paper towels
tts-🧻 = roll of paper
default-🪣 = bucket
tts-🪣 = bucket
default-🧼 = bar
tts-🧼 = soap
default-🫧 = bubbles
tts-🫧 = bubbles
default-🪥 = bathroom
tts-🪥 = toothbrush
default-🧽 = absorbing
tts-🧽 = sponge
default-🧯 = extinguish
tts-🧯 = fire extinguisher
default-🛒 = cart
tts-🛒 = shopping cart
default-🚬 = cigarette
tts-🚬 = cigarette
default-⚰ = coffin
tts-⚰ = coffin
default-🪦 = cemetery
tts-🪦 = headstone
default-⚱ = ashes
tts-⚱ = funeral urn
default-🧿 = bead
tts-🧿 = nazar amulet
default-🪬 = amulet
tts-🪬 = hamsa
default-🗿 = face
tts-🗿 = moai
default-🪧 = demonstration
tts-🪧 = placard
default-🪪 = credentials
tts-🪪 = identification card
default-🏧 = ATM
tts-🏧 = ATM sign
default-🚮 = litter
tts-🚮 = litter in bin sign
default-🚰 = drinking
tts-🚰 = potable water
default-♿ = access
tts-♿ = wheelchair symbol
default-🚹 = bathroom
tts-🚹 = men’s room
default-🚺 = bathroom
tts-🚺 = women’s room
default-🚻 = bathroom
tts-🚻 = restroom
default-🚼 = baby
tts-🚼 = baby symbol
default-🚾 = bathroom
tts-🚾 = water closet
default-🛂 = control
tts-🛂 = passport control
default-🛃 = customs
tts-🛃 = customs
default-🛄 = baggage
tts-🛄 = baggage claim
default-🛅 = baggage
tts-🛅 = left luggage
default-⚠ = warning
tts-⚠ = warning
default-🚸 = child
tts-🚸 = children crossing
default-⛔ = entry
tts-⛔ = no entry
default-🚫 = entry
tts-🚫 = prohibited
default-🚳 = bicycle
tts-🚳 = no bicycles
default-🚭 = forbidden
tts-🚭 = no smoking
default-🚯 = forbidden
tts-🚯 = no littering
default-🚱 = non-drinking
tts-🚱 = non-potable water
default-🚷 = forbidden
tts-🚷 = no pedestrians
default-📵 = cell
tts-📵 = no mobile phones
default-🔞 = 18
tts-🔞 = no one under eighteen
default-☢ = radioactive
tts-☢ = radioactive
default-☣ = biohazard
tts-☣ = biohazard
default-⬆ = arrow
tts-⬆ = up arrow
default-↗ = arrow
tts-↗ = up-right arrow
default-➡ = arrow
tts-➡ = right arrow
default-↘ = arrow
tts-↘ = down-right arrow
default-⬇ = arrow
tts-⬇ = down arrow
default-↙ = arrow
tts-↙ = down-left arrow
default-⬅ = arrow
tts-⬅ = left arrow
default-↖ = arrow
tts-↖ = up-left arrow
default-↕ = arrow
tts-↕ = up-down arrow
default-↔ = arrow
tts-↔ = left-right arrow
default-↮ = left right arrow stroke
tts-↮ = left right arrow stroke
default-↩ = arrow
tts-↩ = right arrow curving left
default-↪ = arrow
tts-↪ = left arrow curving right
default-⤴ = arrow
tts-⤴ = right arrow curving up
default-⤵ = arrow
tts-⤵ = right arrow curving down
default-🔃 = arrow
tts-🔃 = clockwise vertical arrows
default-🔄 = anticlockwise
tts-🔄 = counterclockwise arrows button
default-🔙 = arrow
tts-🔙 = BACK arrow
default-🔚 = arrow
tts-🔚 = END arrow
default-🔛 = arrow
tts-🔛 = ON! arrow
default-🔜 = arrow
tts-🔜 = SOON arrow
default-🔝 = arrow
tts-🔝 = TOP arrow
default-🛐 = place of worship
tts-🛐 = place of worship
default-⚛ = atheist
tts-⚛ = atom symbol
default-🕉 = Hindu
tts-🕉 = om
default-✡ = David
tts-✡ = star of David
default-☸ = Buddhist
tts-☸ = wheel of dharma
default-☯ = religion
tts-☯ = yin yang
default-✝ = Christian
tts-✝ = latin cross
default-☦ = Christian
tts-☦ = orthodox cross
default-☪ = islam
tts-☪ = star and crescent
default-☮ = peace
tts-☮ = peace symbol
default-🕎 = candelabrum
tts-🕎 = menorah
default-🔯 = dotted six-pointed star
tts-🔯 = dotted six-pointed star
default-🪯 = khanda
tts-🪯 = khanda
default-♈ = Aries
tts-♈ = Aries
default-♉ = bull
tts-♉ = Taurus
default-♊ = Gemini
tts-♊ = Gemini
default-♋ = Cancer
tts-♋ = Cancer
default-♌ = Leo
tts-♌ = Leo
default-♍ = Virgo
tts-♍ = Virgo
default-♎ = balance
tts-♎ = Libra
default-♏ = Scorpio
tts-♏ = Scorpio
default-♐ = archer
tts-♐ = Sagittarius
default-♑ = Capricorn
tts-♑ = Capricorn
default-♒ = Aquarius
tts-♒ = Aquarius
default-♓ = fish
tts-♓ = Pisces
default-⛎ = bearer
tts-⛎ = Ophiuchus
default-🔀 = arrow
tts-🔀 = shuffle tracks button
default-🔁 = arrow
tts-🔁 = repeat button
default-🔂 = arrow
tts-🔂 = repeat single button
default-▶ = arrow
tts-▶ = play button
default-⏩ = arrow
tts-⏩ = fast-forward button
default-⏭ = arrow
tts-⏭ = next track button
default-⏯ = arrow
tts-⏯ = play or pause button
default-◀ = arrow
tts-◀ = reverse button
default-⏪ = arrow
tts-⏪ = fast reverse button
default-⏮ = arrow
tts-⏮ = last track button
default-🔼 = arrow
tts-🔼 = upwards button
default-⏫ = arrow
tts-⏫ = fast up button
default-🔽 = arrow
tts-🔽 = downwards button
default-⏬ = arrow
tts-⏬ = fast down button
default-⏸ = bar
tts-⏸ = pause button
default-⏹ = square
tts-⏹ = stop button
default-⏺ = circle
tts-⏺ = record button
default-⏏ = eject
tts-⏏ = eject button
default-🎦 = camera
tts-🎦 = cinema
default-🔅 = brightness
tts-🔅 = dim button
default-🔆 = bright
tts-🔆 = bright button
default-📶 = antenna
tts-📶 = antenna bars
default-🛜 = computer
tts-🛜 = wireless
default-📳 = cell
tts-📳 = vibration mode
default-📴 = cell
tts-📴 = mobile phone off
default-♀ = female sign
tts-♀ = female sign
default-♂ = male sign
tts-♂ = male sign
default-⚧ = transgender
tts-⚧ = transgender symbol
default-✖ = ×
tts-✖ = multiply
default-➕ = +
tts-➕ = plus
default-➖ = -
tts-➖ = minus
default-➗ = ÷
tts-➗ = divide
default-🟰 = equality
tts-🟰 = heavy equals sign
default-♾ = forever
tts-♾ = infinity
default-‼ = !
tts-‼ = double exclamation mark
default-⁉ = !
tts-⁉ = exclamation question mark
default-❓ = ?
tts-❓ = red question mark
default-❔ = ?
tts-❔ = white question mark
default-❕ = !
tts-❕ = white exclamation mark
default-❗ = !
tts-❗ = red exclamation mark
default-〰 = dash
tts-〰 = wavy dash
default-💱 = bank
tts-💱 = currency exchange
default-💲 = currency
tts-💲 = heavy dollar sign
default-⚕ = aesculapius
tts-⚕ = medical symbol
default-♻ = recycle
tts-♻ = recycling symbol
default-⚜ = fleur-de-lis
tts-⚜ = fleur-de-lis
default-🔱 = anchor
tts-🔱 = trident emblem
default-📛 = badge
tts-📛 = name badge
default-🔰 = beginner
tts-🔰 = Japanese symbol for beginner
default-⭕ = circle
tts-⭕ = hollow red circle
default-✅ = ✓
tts-✅ = check mark button
default-☑ = ✓
tts-☑ = check box with check
default-✔ = ✓
tts-✔ = check mark
default-❌ = ×
tts-❌ = cross mark
default-❎ = ×
tts-❎ = cross mark button
default-➰ = curl
tts-➰ = curly loop
default-➿ = curl
tts-➿ = double curly loop
default-〽 = mark
tts-〽 = part alternation mark
default-✳ = *
tts-✳ = eight-spoked asterisk
default-✴ = *
tts-✴ = eight-pointed star
default-❇ = *
tts-❇ = sparkle
default-© = C
tts-© = copyright
default-® = R
tts-® = registered
default-™ = mark
tts-™ = trade mark
default-🔠 = ABCD
tts-🔠 = input latin uppercase
default-🔡 = abcd
tts-🔡 = input latin lowercase
default-🔢 = 1234
tts-🔢 = input numbers
default-🔣 = 〒♪&%
tts-🔣 = input symbols
default-🔤 = abc
tts-🔤 = input latin letters
default-🅰 = A
tts-🅰 = A button (blood type)
default-🆎 = AB
tts-🆎 = AB button (blood type)
default-🅱 = B
tts-🅱 = B button (blood type)
default-🆑 = CL
tts-🆑 = CL button
default-🆒 = COOL
tts-🆒 = COOL button
default-🆓 = FREE
tts-🆓 = FREE button
default-ℹ = i
tts-ℹ = information
default-🆔 = ID
tts-🆔 = ID button
default-Ⓜ = circle
tts-Ⓜ = circled M
default-🆕 = NEW
tts-🆕 = NEW button
default-🆖 = NG
tts-🆖 = NG button
default-🅾 = blood type
tts-🅾 = O button (blood type)
default-🆗 = OK
tts-🆗 = OK button
default-🅿 = P
tts-🅿 = P button
default-🆘 = help
tts-🆘 = SOS button
default-🆙 = mark
tts-🆙 = UP! button
default-🆚 = versus
tts-🆚 = VS button
default-🈁 = “here”
tts-🈁 = Japanese “here” button
default-🈂 = “service charge”
tts-🈂 = Japanese “service charge” button
default-🈷 = “monthly amount”
tts-🈷 = Japanese “monthly amount” button
default-🈶 = “not free of charge”
tts-🈶 = Japanese “not free of charge” button
default-🈯 = “reserved”
tts-🈯 = Japanese “reserved” button
default-🉐 = “bargain”
tts-🉐 = Japanese “bargain” button
default-🈹 = “discount”
tts-🈹 = Japanese “discount” button
default-🈚 = “free of charge”
tts-🈚 = Japanese “free of charge” button
default-🈲 = “prohibited”
tts-🈲 = Japanese “prohibited” button
default-🉑 = “acceptable”
tts-🉑 = Japanese “acceptable” button
default-🈸 = “application”
tts-🈸 = Japanese “application” button
default-🈴 = “passing grade”
tts-🈴 = Japanese “passing grade” button
default-🈳 = “vacancy”
tts-🈳 = Japanese “vacancy” button
default-㊗ = “congratulations”
tts-㊗ = Japanese “congratulations” button
default-㊙ = “secret”
tts-㊙ = Japanese “secret” button
default-🈺 = “open for business”
tts-🈺 = Japanese “open for business” button
default-🈵 = “no vacancy”
tts-🈵 = Japanese “no vacancy” button
default-🔴 = circle
tts-🔴 = red circle
default-🟠 = circle
tts-🟠 = orange circle
default-🟡 = circle
tts-🟡 = yellow circle
default-🟢 = circle
tts-🟢 = green circle
default-🔵 = blue
tts-🔵 = blue circle
default-🟣 = circle
tts-🟣 = purple circle
default-🟤 = brown
tts-🟤 = brown circle
default-⚫ = black circle
tts-⚫ = black circle
default-⚪ = circle
tts-⚪ = white circle
default-🟥 = red
tts-🟥 = red square
default-🟧 = orange
tts-🟧 = orange square
default-🟨 = square
tts-🟨 = yellow square
default-🟩 = green
tts-🟩 = green square
default-🟦 = blue
tts-🟦 = blue square
default-🟪 = purple
tts-🟪 = purple square
default-🟫 = brown
tts-🟫 = brown square
default-⬛ = black large square
tts-⬛ = black large square
default-⬜ = geometric
tts-⬜ = white large square
default-◼ = black medium square
tts-◼ = black medium square
default-◻ = geometric
tts-◻ = white medium square
default-◾ = black medium-small square
tts-◾ = black medium-small square
default-◽ = geometric
tts-◽ = white medium-small square
default-▪ = black small square
tts-▪ = black small square
default-▫ = geometric
tts-▫ = white small square
default-🔶 = diamond
tts-🔶 = large orange diamond
default-🔷 = blue
tts-🔷 = large blue diamond
default-🔸 = diamond
tts-🔸 = small orange diamond
default-🔹 = blue
tts-🔹 = small blue diamond
default-🔺 = geometric
tts-🔺 = red triangle pointed up
default-🔻 = down
tts-🔻 = red triangle pointed down
default-💠 = comic
tts-💠 = diamond with a dot
default-🔘 = button
tts-🔘 = radio button
default-🔳 = button
tts-🔳 = white square button
default-🔲 = black square button
tts-🔲 = black square button
default-🏁 = checkered
tts-🏁 = chequered flag
default-🚩 = post
tts-🚩 = triangular flag
default-🎌 = celebration
tts-🎌 = crossed flags
default-🏴 = black flag
tts-🏴 = black flag
default-🏳 = waving
tts-🏳 = white flag
default-🏳‍🌈 = pride
tts-🏳‍🌈 = rainbow flag
default-🏳‍⚧ = flag
tts-🏳‍⚧ = transgender flag
default-🏴‍☠ = Jolly Roger
tts-🏴‍☠ = pirate flag
default-¢ = cent
tts-¢ = cent
default-$ = dollar
tts-$ = dollar
default-£ = currency
tts-£ = pound
default-¥ = CNY
tts-¥ = yen
default-₢ = BRB
tts-₢ = cruzeiro
default-₣ = currency
tts-₣ = french franc
default-₤ = currency
tts-₤ = lira
default-₥ = mil
tts-₥ = mill
default-₩ = KPW
tts-₩ = won
default-€ = currency
tts-€ = euro
default-₰ = currency
tts-₰ = german penny
default-₱ = peso
tts-₱ = peso
default-₳ = ARA
tts-₳ = austral
default-₶ = currency
tts-₶ = livre tournois
default-₷ = currency
tts-₷ = spesmilo
default-₹ = currency
tts-₹ = indian rupee
default-₽ = currency
tts-₽ = ruble
default-₿ = bitcoin
tts-₿ = bitcoin
default-₨ = currency
tts-₨ = rupee
default-﷼ = currency
tts-﷼ = rial
default-¹ = one
tts-¹ = superscript one
default-² = squared
tts-² = superscript two
default-³ = cubed
tts-³ = superscript three
default-µ = measure
tts-µ = micro sign
